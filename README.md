...
The Products dashboard includes a visible **First catalog launch kit** with a downloadable CSV template, a copyable catalog prep brief, a first-catalog proof path and a shortcut into import. The CSV importer also includes a **Real catalog intake contract** with required/recommended columns, a pre-import checklist, a richer sample template and a copyable handoff packet for preparing the first production catalog.
...
The dashboard **Production Verification Center** also keeps this repair workflow as a fallback reference with copyable steps, a paste-ready repair SQL button, the live verifier command, and the full schema/RLS SQL check so production handoff is not hidden in documentation only. It also includes plain-English proof lanes for what is proven versus still needing evidence, plus a copyable production auth proof template for signup, verification, login, forgot-password, reset-password, and new-password login evidence.
