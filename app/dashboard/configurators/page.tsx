    ...
    <div className="-m-4 flex min-h-[calc(100vh-68px)] flex-col sm:-m-7 lg:-m-9">
      <div className="flex flex-wrap items-center gap-3 border-b border-black/[0.07] bg-white px-4 py-3 sm:px-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-black/35">{draft.steps.length} steps · {allOptions.length} options · {draft.published ? "Published" : "Draft"}</p>
        </div>
      ...
      <div className="grid flex-1 lg:grid-cols-[270px_1fr_330px]">
        <aside className="border-b border-black/[0.07] bg-[#eef0eb] p-4 lg:border-b-0 lg:border-r">
          <p className="px-2 text-xs font-extrabold uppercase tracking-wider text-black/30">Configurator flow</p>
          <button onClick={() => setActiveStepId(null)} className={`mt-3 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-bold ${activeStepId === null ? "bg-white shadow-sm" : "text-black/50 hover:bg-white/60"}`}><span className="grid h-6 w-6 place-items-center rounded-lg bg-lime/55 text-xs">✦</span> Setup</button>
          <div className="mt-3 space-y-1.5">
            {draft.steps.map((step, index) => <button key={step.id} onClick={() => setActiveStepId(step.id)} className={`group flex w-full items-center gap-2 rounded-xl px-2 py-2.5 text-left ${step.id === activeStepId ? "bg-ink text-white shadow-sm" : "text-black/55 hover:bg-white"}`}><GripVertical size={13} className={step.id === activeStepId ? "text-white/30" : "text-black/20"} /><span className={`grid h-6 w-6 shrink-0 place-items-center rounded-lg text-xs font-extrabold ${step.id === activeStepId ? "bg-lime text-ink" : "bg-black/5"}`}>{index + 1}</span><span className="min-w-0 flex-1 truncate text-xs font-bold">{step.title}</span><span className="text-xs opacity-45">{step.options.length}</span></button>)}
          </div>
          ...
          <div className="mt-5 border-t border-black/[0.07] pt-4"><button onClick={async () => { if (confirm("Delete this configurator permanently?")) { await deleteConfigurator(draft.id); onBack(); } }} className="flex items-center gap-2 px-2 py-2 text-xs font-bold text-red-600"><Trash2 size={12} /> Delete configurator</button></div>
        </aside>
        ...
        <main className="min-h-[700px] bg-[#f8f8f5] p-4 sm:p-8 lg:p-10">
          {activeStep ? (
            <div className="mx-auto max-w-3xl">
              ...
              <div className="mt-6 flex items-center justify-between"><div><h3 className="text-sm font-extrabold">Options</h3><p className="mt-1 text-xs text-black/35">Link an option to a product when it should appear in the final recommendation.</p></div><button onClick={() => addOption(activeStep.id)} className="btn-secondary !px-3 !py-2 text-xs"><Plus size={13} /> Add option</button></div>
                  ...
                  return (
                    <div key={option.id} className="rounded-2xl border border-black/[0.07] bg-white p-4 sm:p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-lime/45 text-xs font-extrabold">{String.fromCharCode(65 + index)}</span>
                      ...
                      <div className="mt-4 grid gap-3 border-t border-black/[0.05] pt-4 sm:grid-cols-2">
                        <div><label className="label !text-xs">Linked product</label><select className="field !py-2.5 text-xs" value={option.product_id || ""} onChange={(event) => updateProduct(activeStep.id, option, event.target.value)}><option value="">No linked product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></div>
                        <div><label className="label !text-xs">Price delta</label><input className="field !py-2.5 text-xs" type="number" value={option.price_delta} onChange={(event) => updateOption(activeStep.id, option.id, { price_delta: Number(event.target.value) })} /></div>
                        <div className="sm:col-span-2"><label className="label !text-xs">Description</label><textarea className="field min-h-20 !py-2.5 text-xs" value={option.description} onChange={(event) => updateOption(activeStep.id, option.id, { description: event.target.value })} /></div>
                        <div><label className="label !text-xs">Image URL</label><input className="field !py-2.5 text-xs" value={option.image_url} onChange={(event) => updateOption(activeStep.id, option.id, { image_url: event.target.value })} /></div>
                        <div><label className="label !text-xs">Tags</label><input className="field !py-2.5 text-xs" value={commaList(option.tags)} onChange={(event) => updateOption(activeStep.id, option.id, { tags: parseCommaList(event.target.value) })} placeholder="trail, waterproof, comfort" /></div>
                      </div>
                      {otherOptions.length > 0 && <details className="mt-4 rounded-xl bg-canvas p-3"><summary className="cursor-pointer text-xs font-extrabold text-moss">Compatibility rules</summary><div className="mt-3 grid gap-2 sm:grid-cols-2">{otherOptions.map((other) => <label key={other.id} className="flex items-center gap-2 text-xs font-bold text-black/50"><input type="checkbox" checked={option.incompatible_option_ids.includes(other.id)} onChange={(event) => updateOption(activeStep.id, option.id, { incompatible_option_ids: event.target.checked ? [...option.incompatible_option_ids, other.id] : option.incompatible_option_ids.filter((id) => id !== other.id) })} /> Not compatible with {other.label}</label>)}</div></details>}
                    </div>
                  );
                })}
        ...
        <aside className="hidden border-l border-black/[0.07] bg-white p-5 lg:block">
          ...
          <div className="mt-5 overflow-hidden rounded-[22px] border border-black/10 bg-[#e8eadf] p-3 shadow-sm">
            <div className="rounded-[17px] bg-ink p-4 text-white">
              <div className="flex items-center gap-2 text-xs font-extrabold"><span className="grid h-6 w-6 place-items-center rounded-lg bg-lime text-ink"><Sparkles size={11} /></span> Visual configurator</div>
              <h3 className="display mt-6 text-3xl leading-none">{draft.title}</h3>
              <p className="mt-2 text-xs leading-4 text-white/45">{draft.subtitle}</p>
              <div className="mt-5 rounded-xl bg-white/[.08] p-3"><p className="text-xs font-extrabold text-lime">Preview bundle</p><p className="mt-1 text-xl font-extrabold">{formatCurrency(previewTotal)}</p><div className="mt-3 flex flex-wrap gap-1">{previewSummary.tags.slice(0, 5).map((tag) => <span key={tag} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-lime">{tag}</span>)}</div></div>
            </div>
          </div>
          <div className="mt-5 rounded-xl border border-black/[0.07] p-3">
            ...
            <p className="mt-1.5 text-xs leading-4 text-black/40">{readiness.canPublish ? readiness.warnings.length ? "This configurator can publish, but review warnings before embedding." : "This configurator is structurally ready to publish." : "Fix blockers before shoppers can receive a valid bundle."}</p>
            <div className="mt-3 space-y-1.5">
          ...
          <div className="mt-5 rounded-xl border border-black/[0.07] p-3">
            <div className="flex items-center justify-between gap-3">
              <p className="flex items-center gap-2 text-xs font-extrabold"><Check size={13} className={pathQa.status === "fail" ? "text-red-600" : pathQa.status === "warn" ? "text-amber-600" : "text-moss"} /> Path QA</p>
              <span className={`rounded-full px-2 py-1 text-xs font-extrabold ${pathQa.status === "fail" ? "bg-red-50 text-red-700" : pathQa.status === "warn" ? "bg-amber-50 text-amber-700" : "bg-lime/35 text-moss"}`}>{pathQa.score}%</span>
            </div>
            <p className="mt-1.5 text-xs leading-4 text-black/40">{pathQa.headline}</p>
            <div className="mt-3 grid grid-cols-3 gap-1.5 text-center">
              <div className="rounded-lg bg-canvas px-2 py-2"><p className="text-sm font-extrabold">{pathQa.summary.completionScenarios}</p><p className="text-xs font-bold text-black/30">Paths</p></div>
              <div className="rounded-lg bg-canvas px-2 py-2"><p className="text-sm font-extrabold">{pathQa.summary.compatibilityGuardrails}</p><p className="text-xs font-bold text-black/30">Rules</p></div>
              <div className="rounded-lg bg-canvas px-2 py-2"><p className="text-sm font-extrabold">{pathQa.summary.productLinkedScenarioRate}%</p><p className="text-xs font-bold text-black/30">Linked</p></div>
            </div>
            <div className="mt-3 space-y-1.5">
              {pathQa.scenarios.slice(0, 3).map((scenario) => <div key={scenario.id} className={`rounded-lg px-2.5 py-2 ${scenario.status === "fail" ? "bg-red-50" : scenario.status === "warn" ? "bg-amber-50" : "bg-canvas"}`}>
                <p className="text-xs font-extrabold">{scenario.label}</p>
                <p className="mt-0.5 text-xs font-bold leading-3 text-black/35">{scenario.detail}</p>
              </div>)}
              {!pathQa.scenarios.length && <p className="rounded-lg bg-canvas px-2.5 py-2 text-xs font-bold leading-3 text-black/35">Add options before path QA can simulate shopper bundles.</p>}
            </div>
          </div>
          <div className="mt-5 rounded-xl border border-black/[0.07] p-3"><p className="flex items-center gap-2 text-xs font-extrabold"><Settings2 size={13} className="text-moss" /> Builder tip</p><p className="mt-1.5 text-xs leading-4 text-black/40">Use linked products for the main purchasable items. Use unlinked options for preferences, materials, service plans or add-ons.</p></div>
        </aside>
      </div>
    </div>
  );
              ...
              <article key={configurator.id} className="group overflow-hidden rounded-2xl border border-black/[0.07] bg-white">
                <button onClick={() => setSelectedId(configurator.id)} className="relative block h-48 w-full overflow-hidden bg-ink p-5 text-left text-white">
                  ...
                  <div className="relative flex h-full flex-col">
                    <span className={`self-end rounded-full px-2 py-1 text-xs font-extrabold ${configurator.published ? "bg-lime text-moss" : "bg-white/15 text-white/55"}`}>{configurator.published ? "Live" : "Draft"}</span>
                ...
                <div className="p-5">
                  ...
                  <p className="mt-1 text-xs text-black/35">{configurator.steps.length} steps · {options} options</p>
                  <div className="mt-5 grid grid-cols-4 divide-x divide-black/[0.07] rounded-xl bg-canvas py-3 text-center">
                    <div><p className="text-sm font-extrabold">{views}</p><p className="mt-0.5 text-xs font-bold text-black/30">Views</p></div>
                    <div><p className="text-sm font-extrabold">{completed}</p><p className="mt-0.5 text-xs font-bold text-black/30">Complete</p></div>
                    <div><p className="text-sm font-extrabold">{views ? Math.round(completed / views * 100) : 0}%</p><p className="mt-0.5 text-xs font-bold text-black/30">Rate</p></div>
                    <div><p className="text-sm font-extrabold">{qa.score}%</p><p className="mt-0.5 text-xs font-bold text-black/30">QA</p></div>
                  </div>
