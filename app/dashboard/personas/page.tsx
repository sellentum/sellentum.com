      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[380px_1fr]">
        <section className="rounded-[30px] border border-black/[0.07] bg-ink p-7 text-white">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.personas}</p><p className="mt-1 text-xs text-white/35">Personas</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.highConfidence}</p><p className="mt-1 text-xs text-white/35">High conf.</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.journeySessions}</p><p className="mt-1 text-xs text-white/35">Sessions</p></div>
          </div>
        </section>
              ...
              {report.personas.map((persona) => {
                ...
                return (
                  <article key={persona.id} className="rounded-[24px] border border-black/[0.07] bg-canvas p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-extrabold uppercase tracking-wider text-moss">{persona.segment}</p>
                        <h3 className="mt-2 text-xl font-extrabold tracking-[-.045em]">{persona.name}</h3>
                        <p className="mt-2 text-xs leading-4 text-black/45">{persona.description}</p>
                      </div>
                      <span className={cn("rounded-full px-3 py-1.5 text-xs font-extrabold uppercase", confidenceTone[persona.confidence])}>{persona.confidence}</span>
                    </div>
                    ...
                    <div className="mt-5 grid grid-cols-4 gap-2 text-center">
                      ...
                      ].map(([value, label]) => <div key={String(label)} className="rounded-2xl bg-white p-3"><p className="text-lg font-extrabold">{String(value)}</p><p className="mt-1 text-xs text-black/30">{String(label)}</p></div>)}
                    </div>

                    <div className="mt-4 rounded-2xl bg-white p-4">
                      <p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-moss"><Icon size={13} /> Recommended play</p>
                      <p className="mt-2 text-xs leading-4 text-black/50">{persona.launchAngle}</p>
                      <p className="mt-3 rounded-xl bg-canvas px-3 py-2 text-xs font-bold leading-4 text-black/40">{persona.evidence}</p>
                    </div>

                    <div className="mt-4 grid gap-3 xl:grid-cols-2">
                      <div>
                        <p className="text-xs font-extrabold uppercase tracking-wider text-black/30">Top signals</p>
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {[...persona.intentSignals, ...persona.answerSignals, ...persona.querySignals].slice(0, 7).map((signal) => <span key={`${persona.id}-${signal}`} className="rounded-full bg-white px-2 py-1 text-xs font-extrabold text-black/35">{signal}</span>)}
                        </div>
                      </div>
                      <div>
                        <p className="text-xs font-extrabold uppercase tracking-wider text-black/30">Product affinities</p>
                        <div className="mt-2 space-y-1.5">
                          {persona.productAffinities.slice(0, 3).map((product) => (
                            <div key={`${persona.id}-${product.productName}`} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-xs font-bold text-black/45">
                              <span className="truncate">{product.productName}</span>
                              <span className="shrink-0 text-moss">{product.clicks} clicks</span>
                            </div>
                          ))}
                          {!persona.productAffinities.length && <p className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-black/35">No product affinity yet.</p>}
                        </div>
                      </div>
                    </div>
              ...
              {report.signalMatrix.map((row) => (
                <article key={row.id} className="rounded-2xl border border-black/[0.07] bg-canvas p-4">
                  <div className="flex items-center justify-between gap-3">
                    <span className={cn("rounded-full px-2.5 py-1 text-xs font-extrabold uppercase", matrixTone[row.status])}>{row.status}</span>
                  ...
                  <p className="mt-2 text-xs leading-4 text-black/45">{row.detail}</p>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {row.examples.map((example) => <span key={`${row.id}-${example}`} className="rounded-full bg-white px-2 py-1 text-xs font-extrabold text-black/35">{example}</span>)}
                  </div>
                </article>
              ))}
            </div>
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-center">
              <div className="rounded-2xl bg-canvas p-4"><UsersRound className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.personas}</p><p className="mt-1 text-xs font-bold text-black/30">Segments</p></div>
              <div className="rounded-2xl bg-canvas p-4"><Tags className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.explicitSignals}</p><p className="mt-1 text-xs font-bold text-black/30">Signals</p></div>
              <div className="rounded-2xl bg-canvas p-4"><MousePointerClick className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.averageConversionRate}%</p><p className="mt-1 text-xs font-bold text-black/30">Avg CVR</p></div>
              <div className="rounded-2xl bg-canvas p-4"><ExternalLink className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.actions.length}</p><p className="mt-1 text-xs font-bold text-black/30">Actions</p></div>
            </div>
          </section>
        </aside>
      </div>
