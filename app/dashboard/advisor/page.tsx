      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[390px_1fr]">
        <section className="rounded-[30px] border border-black/[0.07] bg-ink p-7 text-white">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.recommendationPrompts}</p><p className="mt-1 text-[8px] text-white/35">Recommend</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.clarifyingPrompts}</p><p className="mt-1 text-[8px] text-white/35">Clarify</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.weakPrompts + report.summary.noResultPrompts}</p><p className="mt-1 text-[8px] text-white/35">Tune</p></div>
          </div>
        </section>
        ...
        <div className="grid gap-0 xl:grid-cols-[1fr_380px]">
          <div className="p-6 sm:p-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[10px] text-white/35">This mirrors the deterministic intent and recovery checks behind the public advisor runtime.</p>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {report.scenarios.slice(0, 6).map((item) => (
                <button key={item.id} onClick={() => setPrompt(item.prompt)} className={cn("rounded-full border px-3 py-1.5 text-[10px] font-extrabold transition", prompt === item.prompt ? "border-lime bg-lime text-ink" : "border-white/10 bg-white/[0.06] text-white/55 hover:bg-white/10")}>{item.prompt}</button>
              ))}
            </div>
          </div>
          <aside className="border-t border-white/10 bg-white/[0.04] p-6 xl:border-l xl:border-t-0">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-extrabold">{scenario.terms.length}</p><p className="mt-1 text-[8px] font-bold uppercase tracking-wider text-white/30">Intent terms</p></div>
              <div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-extrabold">{scenario.maxBudget ? formatCurrency(scenario.maxBudget) : "—"}</p><p className="mt-1 text-[8px] font-bold uppercase tracking-wider text-white/30">Budget</p></div>
            </div>
            <div className="mt-4 flex flex-wrap gap-1.5">
              {scenario.terms.slice(0, 12).map((term) => <span key={term} className="rounded-full bg-lime/15 px-2 py-1 text-[8px] font-extrabold text-lime">{term}</span>)}
              {!scenario.terms.length && <span className="text-[10px] font-bold text-white/35">Try a more specific shopper request.</span>}
            </div>
          </aside>
        </div>
      </section>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_420px]">
        <main className="space-y-5">
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className={cn("rounded-full px-3 py-1.5 text-[9px] font-extrabold uppercase", statusTone[scenario.status])}>{scenario.status}</span>
            </div>
              ...
              <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-[10px] leading-4 text-black/45">{scenario.recovery.summary}</p>
                  </div>
                  <span className="rounded-full bg-white px-2.5 py-1 text-[8px] font-extrabold uppercase text-amber-700">{scenario.recovery.status}</span>
                </div>
                <div className="mt-4 grid gap-3 xl:grid-cols-2">
                  {scenario.recovery.suggestions.map((suggestion) => (
                    <button key={suggestion.id} onClick={() => suggestion.prompt && setPrompt(suggestion.prompt)} className="rounded-xl bg-white p-3 text-left">
                      ...
                      {suggestion.prompt && <p className="mt-2 text-[9px] font-extrabold text-moss">Try: {suggestion.prompt}</p>}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {scenario.results.map((result, index) => (
                <article key={result.product.id} className="grid gap-5 rounded-2xl border border-black/[0.07] p-4 xl:grid-cols-[260px_1fr]">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn("rounded-full px-2 py-1 text-[8px] font-extrabold", result.confidence === "strong" ? "bg-moss text-white" : result.confidence === "medium" ? "bg-amber-100 text-amber-700" : "bg-black/5 text-black/35")}>{result.confidence}</span>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {result.matchedSignals.slice(0, 8).map((signal) => <span key={`${result.product.id}-${signal.term}-${signal.source}`} className="rounded-full bg-canvas px-2 py-1 text-[8px] font-extrabold text-black/40">{signal.term} · {signal.source} +{signal.contribution.toFixed(1)}</span>)}
                    </div>
                  </div>
                </article>
              ))}
              {!scenario.results.length && <div className="rounded-2xl border border-dashed border-black/10 bg-canvas p-8 text-center"><p className="text-xs font-extrabold">No deterministic recommendations yet</p><p className="mt-1 text-[10px] text-black/35">Use the recovery prompts above or enrich the catalog terms behind this request.</p></div>}
            </div>
          </section>
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-6">
            ...
            <div className="mt-5 grid gap-2 xl:grid-cols-2">
              {scenario.coverage.map((item) => (
                <div key={item.term} className={cn("rounded-xl border px-3 py-2", item.status === "covered" ? "border-lime/50 bg-lime/15" : item.status === "thin" ? "border-amber-200 bg-amber-50" : "border-red-100 bg-red-50")}>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className={cn("rounded-full px-2 py-0.5 text-[8px] font-extrabold uppercase", item.status === "covered" ? "bg-lime text-moss" : item.status === "thin" ? "bg-amber-200 text-amber-800" : "bg-red-100 text-red-600")}>{item.status}</span>
                  </div>
              ...
              {!scenario.coverage.length && <p className="rounded-xl bg-canvas px-3 py-5 text-center text-[10px] leading-4 text-black/35 xl:col-span-2">Enter a more specific shopper prompt to inspect catalog coverage.</p>}
            </div>
          </section>
        </main>
        ...
        <aside className="space-y-5">
                ...
                return (
                  <Link key={check.id} href={check.href} className="block rounded-2xl bg-canvas p-4 transition hover:bg-white">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2"><h3 className="text-xs font-extrabold">{check.label}</h3><span className={cn("rounded-full px-2 py-0.5 text-[8px] font-extrabold uppercase", checkTone[check.status])}>{check.status}</span></div>
                        <p className="mt-2 text-[10px] leading-4 text-black/45">{check.detail}</p>
                        <p className="mt-3 rounded-xl bg-white px-3 py-2 text-[9px] font-bold leading-4 text-black/45">{check.evidence}</p>
                      </div>
                    </div>
                  </Link>
                );
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-ink p-5 text-white">
            ...
            <pre className="mt-4 max-h-56 overflow-auto rounded-2xl bg-black/25 p-3 text-[9px] leading-4 text-white/55">{report.snippet}</pre>
