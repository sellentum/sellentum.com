...
import { Boxes, Check, ChevronDown, Download, FileSpreadsheet, ImageIcon, LoaderCircle, MoreHorizontal, Pencil, Plus, Search, Sparkles, Trash2, Upload } from "lucide-react";
...
const blankProduct: ProductInput = { name: "", price: 0, image_url: "", category: "", description: "", features: [], tags: [], product_url: "", active: true };

function ProductForm({ product, onClose }: { product?: Product; onClose: () => void }) {
  const { saveProduct } = useStore();
  const [form, setForm] = useState<ProductInput>(product ? { name: product.name, price: product.price, image_url: product.image_url, category: product.category, description: product.description, features: product.features, tags: product.tags, product_url: product.product_url, active: product.active } : blankProduct);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try { await saveProduct({ ...form, features: uniqueValues(featureText.split(",")), tags: uniqueValues(tagText.split(",")) }, product?.id); onClose(); }
...
function CsvImport({ onClose }: { onClose: () => void }) {
  ...
  const template = "name,price,image_url,category,description,features,tags,product_url,active\nExample Product,99,https://example.com/image.jpg,Category,A short description,Feature one|Feature two,tag-one|tag-two,https://example.com/product,true";
...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => products.filter((product) => (category === "All categories" || product.category === category) && `${product.name} ${product.category} ${product.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase())), [products, category, query]);
