...
function ProductForm({ product, onClose }: { product?: Product; onClose: () => void }) {
  ...
  return <form onSubmit={submit}>
    <div className="grid gap-5 p-5 sm:p-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><div><label className="label">Features</label><input className="field" value={featureText} onChange={(e) => setFeatureText(e.target.value)} placeholder="Lightweight, Water resistant" /><p className="mt-1 text-xs text-black/30">Separate values with commas</p></div><div><label className="label">Tags</label><input className="field" value={tagText} onChange={(e) => setTagText(e.target.value)} placeholder="trail, outdoors, premium" /><p className="mt-1 text-xs text-black/30">Used by your recommendation rules</p></div></div>
      <div className="grid gap-4 sm:grid-cols-2"><div><label className="label">Buyer needs</label><input className="field" value={buyerNeedText} onChange={(e) => setBuyerNeedText(e.target.value)} placeholder="wet-weather protection, all-day comfort" /><p className="mt-1 text-xs text-black/30">Shopper-friendly outcomes used by search, advisor and quiz generation</p></div><div><label className="label">Semantic search text</label><textarea className="field min-h-[86px] resize-y" value={form.search_text || ""} onChange={(e) => update("search_text", e.target.value)} placeholder="Extra discovery language, synonyms, use cases or benefit copy" /></div></div>
      <label className="flex items-center justify-between rounded-xl border border-black/[0.07] p-3.5"><span><span className="block text-xs font-extrabold">Available for recommendations</span><span className="mt-0.5 block text-xs text-black/35">Inactive products stay in your catalog but are never suggested.</span></span><input type="checkbox" className="h-4 w-4 accent-ink" checked={form.active} onChange={(e) => update("active", e.target.checked)} /></label>
  ...
  return <div className="p-5 sm:p-7">
    ...
    {result && <div className="mt-5 max-h-[320px] overflow-y-auto rounded-2xl border border-black/[0.07]">
      <table className="w-full min-w-[620px] text-left">
        <thead><tr className="border-b border-black/[0.06] bg-canvas text-xs font-extrabold uppercase tracking-wider text-black/35"><th className="px-4 py-3">Row</th><th className="px-4 py-3">Product</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Notes</th></tr></thead>
        <tbody>
          {result.rows.slice(0, 25).map((row) => <tr key={row.rowNumber} className="border-b border-black/[0.05] last:border-0">
            <td className="px-4 py-3 text-xs font-extrabold text-black/35">{row.rowNumber}</td>
            <td className="px-4 py-3"><p className="text-xs font-extrabold">{row.product?.name || row.sourceName}</p><p className="mt-0.5 text-xs text-black/35">{row.product ? `${row.product.category} · ${formatCurrency(row.product.price)}` : "Not importable yet"}</p></td>
            <td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-extrabold ${row.status === "valid" ? "bg-lime/35 text-moss" : "bg-red-50 text-red-600"}`}>{row.status === "valid" ? "Valid" : "Fix required"}</span></td>
            <td className="px-4 py-3"><div className="space-y-1">{[...row.issues, ...row.warnings].slice(0, 3).map((note) => <p key={note} className={`text-xs font-bold leading-4 ${row.issues.includes(note) ? "text-red-600" : "text-black/35"}`}>{note}</p>)}</div></td>
          </tr>)}
        </tbody>
      </table>
      {result.rows.length > 25 && <p className="border-t border-black/[0.06] bg-canvas px-4 py-3 text-xs font-bold text-black/35">Showing first 25 of {result.rows.length} parsed rows.</p>}
    </div>}
    ...
    <section className="mt-8 grid gap-5 xl:grid-cols-[380px_1fr]">
      <div className="rounded-[28px] border border-black/[0.07] bg-ink p-6 text-white">
        ...
        <div className="mt-6 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-2xl bg-white/[.07] p-3"><p className="text-xl font-extrabold">{intelligence.activeProducts}</p><p className="mt-1 text-xs text-white/35">Active</p></div>
          <div className="rounded-2xl bg-white/[.07] p-3"><p className="text-xl font-extrabold">{intelligence.discoveryReadyProducts}</p><p className="mt-1 text-xs text-white/35">Ready</p></div>
          <div className="rounded-2xl bg-white/[.07] p-3"><p className="text-xl font-extrabold">{intelligence.enrichedProducts}</p><p className="mt-1 text-xs text-white/35">Enriched</p></div>
        </div>
      </div>
      <div className="rounded-[28px] border border-black/[0.07] bg-white p-5 sm:p-6">
        <div className="flex items-center justify-between gap-4"><div><h2 className="flex items-center gap-2 text-sm font-extrabold"><Tags size={16} className="text-moss" /> Discovery readiness</h2><p className="mt-1 text-xs leading-4 text-black/35">The same catalog health model feeds Launch Preflight.</p></div><button onClick={() => setEnrichOpen(true)} disabled={!products.length} className="btn-secondary !px-3 !py-2 text-xs"><Sparkles size={12} /> Enrich</button></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {[["Descriptions", intelligence.coverage.descriptions], ["Signals", intelligence.coverage.matchingSignals], ["Search text", intelligence.coverage.searchText], ["Enrichment", intelligence.coverage.enrichment], ["Images", intelligence.coverage.images], ["Product URLs", intelligence.coverage.productUrls]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-canvas p-3"><div className="flex items-center justify-between text-xs font-extrabold text-black/35"><span>{String(label)}</span><span>{String(value)}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/5"><div className="h-full rounded-full bg-lime" style={{ width: `${value}%` }} /></div></div>)}
        </div>
        <div className="mt-5 grid gap-2 lg:grid-cols-2">
          {intelligence.checks.slice(0, 4).map((item) => <div key={item.id} className={`rounded-2xl border p-3 ${item.severity === "pass" ? "border-lime/40 bg-lime/10" : item.severity === "warning" ? "border-amber-100 bg-amber-50" : "border-red-100 bg-red-50"}`}><p className="flex items-center gap-2 text-xs font-extrabold">{item.severity === "pass" ? <Check size={12} className="text-moss" /> : <AlertTriangle size={12} className={item.severity === "warning" ? "text-amber-600" : "text-red-600"} />}{item.label}</p><p className="mt-1 text-xs font-bold leading-4 text-black/45">{item.detail}</p></div>)}
        </div>
      </div>
    </section>
