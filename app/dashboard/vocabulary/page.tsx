      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[360px_1fr]">
        <section className="rounded-[28px] border border-black/[0.07] bg-ink p-6 text-white">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.approvedTerms}</p><p className="mt-1 text-[8px] text-white/35">Approved</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.reviewTerms}</p><p className="mt-1 text-[8px] text-white/35">Review</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.missingTerms}</p><p className="mt-1 text-[8px] text-white/35">Missing</p></div>
          </div>
        </section>
              ...
              {report.terms.slice(0, 12).map((term) => (
                <article key={term.term} className="rounded-2xl border border-black/[0.07] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold">{term.label}</h3><p className="mt-1 text-[9px] font-bold text-black/30">{term.sourceLabel}</p></div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", itemTone[term.status])}>{term.status}</span>
                  </div>
                  <p className="mt-3 text-[10px] leading-4 text-black/45">{term.reviewNote}</p>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="rounded-full bg-canvas px-2 py-1 text-[8px] font-extrabold text-black/35">{term.productCount} direct</span>
                    <span className="rounded-full bg-canvas px-2 py-1 text-[8px] font-extrabold text-black/35">{term.semanticProductCount} semantic</span>
                    {term.canonicalSignal && <span className="rounded-full bg-lime/25 px-2 py-1 text-[8px] font-extrabold text-moss">{term.canonicalSignal.label}</span>}
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="mt-5 grid gap-3 xl:grid-cols-2">
              {report.unsupportedTerms.map((term) => (
                <article key={term.term} className="rounded-2xl bg-red-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold text-red-800">{term.label}</h3><p className="mt-1 text-[9px] font-bold text-red-700/60">{term.exampleQueries[0] || "No query sample"}</p></div>
                    <AlertTriangle size={15} className="text-red-600" />
                  </div>
                  <p className="mt-3 text-[10px] font-bold leading-4 text-red-700">{term.reviewNote}</p>
                </article>
              ))}
            ...
            <div className="mt-5 space-y-3">
              {report.synonymClusters.slice(0, 8).map((cluster) => (
                <article key={cluster.id} className="rounded-2xl border border-black/[0.07] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold">{cluster.anchor}</h3><p className="mt-1 text-[10px] leading-4 text-black/40">{cluster.recommendation}</p></div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", itemTone[cluster.status])}>{cluster.status}</span>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {cluster.synonyms.map((synonym) => <span key={`${cluster.id}-${synonym.term}`} className={cn("rounded-full px-2 py-1 text-[8px] font-extrabold", itemTone[synonym.status])}>{synonym.term} · {synonym.productCount}</span>)}
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="mt-4 space-y-2">
              {report.productTasks.slice(0, 5).map((task) => (
                <article key={task.productId} className="rounded-2xl bg-white/[0.06] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold">{task.productName}</h3><p className="mt-1 text-[9px] font-bold text-white/35">{task.missingFields.join(", ") || "Review language"}</p></div>
                    <span className={cn("rounded-full px-2 py-1 text-[8px] font-extrabold uppercase", itemTone[task.status])}>{task.score}%</span>
                  </div>
                  <p className="mt-3 line-clamp-3 text-[10px] font-bold leading-4 text-white/45">{task.suggestedSearchText}</p>
                </article>
              ))}
              {!report.productTasks.length && <p className="rounded-2xl bg-lime/10 p-4 text-[10px] font-bold leading-4 text-lime">All products have enough semantic language for launch QA.</p>}
            </div>
          </section>
            ...
            <div className="mt-4 space-y-2">
              {report.quizOpportunities.map((item) => (
                <article key={item.id} className="rounded-2xl bg-canvas p-4">
                  ...
                  <p className="mt-3 rounded-xl bg-white px-3 py-2 text-[9px] font-bold leading-4 text-black/45">{item.suggestedQuestion}</p>
                  <div className="mt-2 flex flex-wrap gap-1">{item.suggestedOptions.map((option) => <span key={`${item.id}-${option}`} className="rounded-full bg-white px-2 py-1 text-[8px] font-extrabold text-black/35">{option}</span>)}</div>
                </article>
              ))}
              {!report.quizOpportunities.length && <p className="rounded-2xl bg-canvas p-4 text-[10px] font-bold leading-4 text-black/35">No vocabulary-driven quiz updates needed right now.</p>}
            </div>
          </section>
        </aside>
      </div>
