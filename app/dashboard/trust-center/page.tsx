      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[380px_1fr]">
        <section className="rounded-[30px] border border-black/[0.07] bg-ink p-7 text-white">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.passing}</p><p className="mt-1 text-[8px] text-white/35">Pass</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.warnings}</p><p className="mt-1 text-[8px] text-white/35">Review</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.blockers}</p><p className="mt-1 text-[8px] text-white/35">Blockers</p></div>
          </div>
        </section>
        ...
        <main className="space-y-5">
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded-full bg-lime/35 px-3 py-1.5 text-[9px] font-extrabold uppercase text-moss">Rules select. AI explains.</span>
            </div>
            <div className="mt-5 grid gap-3 xl:grid-cols-2">
              {report.principles.map((principle) => (
                <article key={principle.label} className="rounded-2xl border border-black/[0.07] bg-canvas p-4">
                  ...
                  <p className="mt-2 text-[10px] leading-4 text-black/45">{principle.detail}</p>
                  <p className="mt-3 rounded-xl bg-white px-3 py-2 text-[9px] font-bold leading-4 text-black/40">{principle.proof}</p>
                </article>
              ))}
            </div>
          </section>
                  ...
                  <Link key={pillar.id} href={pillar.href} className="rounded-2xl border border-black/[0.07] p-4 transition hover:bg-canvas">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div><h3 className="text-xs font-extrabold">{pillar.label}</h3><p className="mt-1 text-[9px] font-extrabold uppercase text-black/30">{pillar.score}% · {pillar.status}</p></div>
                      </div>
                      <ArrowRight size={14} className="text-black/25" />
                    </div>
                    <p className="mt-3 text-[10px] leading-4 text-black/45">{pillar.detail}</p>
                    <div className="mt-3 space-y-1.5">
                      {pillar.evidence.map((item) => <p key={`${pillar.id}-${item}`} className="rounded-xl bg-canvas px-3 py-2 text-[9px] font-bold leading-4 text-black/40">{item}</p>)}
                    </div>
                  </Link>
                );
              })}
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-2">
              <article className="rounded-2xl bg-ink p-5 text-white">
                ...
                <div className="mt-4 space-y-2">
                  {report.aiBoundary.map((item) => <p key={item} className="rounded-xl bg-white/[0.06] px-3 py-2 text-[10px] font-bold leading-4 text-white/55">{item}</p>)}
                </div>
              </article>
              <article className="rounded-2xl bg-[#f8f8f4] p-5">
                ...
                <div className="mt-4 space-y-2">
                  {report.dataBoundary.map((item) => <p key={item} className="rounded-xl bg-white px-3 py-2 text-[10px] font-bold leading-4 text-black/45">{item}</p>)}
                </div>
              </article>
            </div>
          </section>
        ...
        <aside className="space-y-5">
            ...
            <div className="mt-4 space-y-3">
              {report.actions.map((action) => (
                <Link key={action.id} href={action.href} className="block rounded-2xl bg-canvas p-4 transition hover:bg-white">
                  <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", actionTone[action.priority])}>{action.priority}</span>
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-center">
              <div className="rounded-2xl bg-canvas p-4"><p className="text-2xl font-extrabold">{report.summary.pillars}</p><p className="mt-1 text-[8px] font-bold text-black/30">Pillars</p></div>
              <div className="rounded-2xl bg-canvas p-4"><p className="text-2xl font-extrabold">{report.summary.decisionGraphScore}%</p><p className="mt-1 text-[8px] font-bold text-black/30">Graph</p></div>
              <div className="rounded-2xl bg-canvas p-4"><p className="text-2xl font-extrabold">{report.summary.analyticsQualityScore}%</p><p className="mt-1 text-[8px] font-bold text-black/30">Analytics</p></div>
              <div className="rounded-2xl bg-canvas p-4"><p className="text-2xl font-extrabold">{report.summary.groundedRecommendations}</p><p className="mt-1 text-[8px] font-bold text-black/30">Grounded</p></div>
            </div>
          </section>
        </aside>
      </div>
