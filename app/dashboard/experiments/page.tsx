      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[320px_1fr]">
        <section className="rounded-[28px] border border-black/[0.07] bg-ink p-6 text-white">
          <div className="flex items-center justify-between">
            ...
            <span className={cn("rounded-full px-3 py-1.5 text-[9px] font-extrabold uppercase", report.status === "blocked" ? "bg-red-500/20 text-red-100" : report.status === "learning" ? "bg-blue-400/20 text-blue-100" : "bg-lime text-ink")}>{report.status}</span>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", active ? "bg-white/10 text-white/45" : statusTone[experiment.status])}>{experiment.status}</span>
                  <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", active ? "bg-lime text-ink" : priorityTone[experiment.priority])}>{experiment.priority}</span>
                </div>
                ...
                <div className={cn("mt-4 rounded-xl p-3", active ? "bg-white/[0.06]" : "bg-canvas")}>
                  <p className={cn("text-[8px] font-extrabold uppercase tracking-wider", active ? "text-white/25" : "text-black/25")}>{experiment.primaryMetric.label}</p>
            ...
            <div className="flex items-start justify-between gap-8">
              <div>
                <div className="flex gap-2">
                  <span className={cn("rounded-full px-3 py-1.5 text-[9px] font-extrabold uppercase", statusTone[selected.status])}>{selected.status}</span>
                  <span className={cn("rounded-full px-3 py-1.5 text-[9px] font-extrabold uppercase", priorityTone[selected.priority])}>{selected.priority}</span>
                </div>
                ...
                <div className="mt-5 grid gap-3 xl:grid-cols-3">
                  {[selected.primaryMetric, ...selected.secondaryMetrics].map((metricItem) => (
                    <div key={metricItem.label} className="rounded-2xl bg-canvas p-4">
                      <p className="text-[9px] font-extrabold uppercase tracking-wider text-black/30">{metricItem.label}</p>
                      <p className="display mt-3 text-3xl">{metricItem.current}{metricItem.unit}</p>
                      <p className="mt-1 text-[10px] font-bold text-moss">Target {metricItem.target}{metricItem.unit}</p>
                      <p className="mt-2 text-[9px] leading-4 text-black/40">{metricItem.detail}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section className="rounded-2xl border border-black/[0.07] p-5">
                ...
                <div className="mt-4 space-y-2">
                  {selected.setupSteps.map((step, index) => (
                    <p key={step} className="flex gap-3 rounded-xl bg-canvas p-3 text-xs font-bold leading-5 text-black/50"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-lime text-[10px] font-extrabold text-ink">{index + 1}</span>{step}</p>
                  ))}
                </div>
              </section>
            </div>
            ...
            <div className="space-y-6">
                  ...
                  {report.guardrails.map((guardrail) => (
                    <div key={guardrail.id} className="rounded-xl bg-white/[0.06] p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className={cn("rounded-full px-2 py-1 text-[8px] font-extrabold uppercase", guardrail.status === "pass" ? "bg-lime text-ink" : guardrail.status === "warn" ? "bg-amber-300/20 text-amber-100" : "bg-red-400/20 text-red-100")}>{guardrail.status}</span>
                      </div>
                      <p className="mt-1 text-[9px] leading-4 text-white/35">{guardrail.detail}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section className="rounded-2xl border border-black/[0.07] p-5">
                ...
                <div className="mt-5 rounded-2xl bg-canvas p-4">
                  ...
                  <p className="mt-2 text-[10px] leading-4 text-black/45">{selected.rollbackPlan}</p>
                </div>
              </section>
              ...
              <section className="rounded-2xl border border-black/[0.07] bg-[#f8f8f4] p-5">
                ...
                {selected.blockers.length ? <div className="mt-4 space-y-2">{selected.blockers.map((blocker) => <p key={blocker} className="rounded-xl bg-red-50 p-3 text-[10px] font-bold leading-4 text-red-700">{blocker}</p>)}</div> : <p className="mt-4 rounded-xl bg-lime/15 p-3 text-[10px] font-bold leading-4 text-moss">No launch blockers detected for this experiment.</p>}
              </section>
            </div>
          </div>
        </main>
