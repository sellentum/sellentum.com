      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[390px_1fr]">
        <section className="rounded-[30px] border border-black/[0.07] bg-ink p-7 text-white">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.productsAtRisk}</p><p className="mt-1 text-[8px] text-white/35">At risk</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.zeroResultJourneys}</p><p className="mt-1 text-[8px] text-white/35">No result</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{Math.round(report.summary.assistedClickRate)}%</p><p className="mt-1 text-[8px] text-white/35">Click rate</p></div>
          </div>
        </section>
                  ...
                  <div className="flex items-start justify-between gap-5">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", riskTone[product.riskLevel])}>{product.riskLevel} risk · {product.riskScore}%</span>
                      </div>
                      <p className="mt-1 text-xs font-bold text-black/35">{product.category} · {formatCurrency(product.price)}</p>
                      <p className="mt-3 max-w-3xl text-[10px] font-bold leading-4 text-moss">{product.preventionPlay}</p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-2 xl:grid-cols-3">
                    {product.drivers.length ? product.drivers.slice(0, 3).map((driver) => (
                      <div key={`${product.productId}-${driver.id}`} className={cn("rounded-2xl p-3", riskTone[driver.severity])}>
                        <p className="text-[10px] font-extrabold">{driver.label}</p>
                        <p className="mt-1 text-[9px] leading-4 opacity-70">{driver.detail}</p>
                      </div>
                    )) : <div className="rounded-2xl bg-lime/15 p-3 text-[10px] font-bold leading-4 text-moss">No urgent fit-risk drivers detected for this product.</div>}
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="mt-5 grid gap-3 xl:grid-cols-3">
              {report.questionGaps.map((gap) => (
                <Link key={gap.id} href={gap.actionHref} className="rounded-2xl bg-canvas p-4 transition hover:-translate-y-0.5 hover:bg-white">
                  <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", gap.severity === "critical" || gap.severity === "high" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700")}>{gap.severity}</span>
                  <h3 className="mt-4 text-xs font-extrabold leading-5">{gap.title}</h3>
                  <p className="mt-2 text-[10px] leading-4 text-black/40">{gap.detail}</p>
                  <p className="mt-3 rounded-xl bg-white px-3 py-2 text-[10px] font-bold leading-4 text-black/55">{gap.suggestedQuestion}</p>
                  <div className="mt-3 flex flex-wrap gap-1.5">{gap.answerOptions.map((option) => <span key={option} className="rounded-full bg-white px-2 py-1 text-[8px] font-bold text-black/40">{option}</span>)}</div>
                </Link>
              ))}
              ...
              {report.frictionSignals.map((signal) => (
                <Link key={signal.id} href={signal.actionHref} className="flex items-start gap-3 rounded-2xl bg-canvas p-4 transition hover:bg-white">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block text-[10px] leading-4 text-black/40">{signal.detail}</span>
                    <span className="mt-2 inline-flex rounded-full bg-white px-2 py-1 text-[8px] font-extrabold uppercase text-black/35">{signal.count} · {signal.source}</span>
                  </span>
                </Link>
              ))}
            </div>
              ...
              {report.scripts.map((script) => (
                <div key={script.id} className="rounded-2xl bg-canvas p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-white px-2 py-1 text-[8px] font-extrabold uppercase text-black/35">{script.audience}</span>
                  </div>
                  <p className="mt-2 text-[10px] font-bold leading-4 text-black/45">{script.script}</p>
                </div>
              ))}
            </div>
          </section>
