      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[360px_1fr]">
        <section className="rounded-[28px] border border-black/[0.07] bg-ink p-6 text-white">
          <div className="flex items-center justify-between">
            ...
            <span className={cn("rounded-full px-3 py-1.5 text-[9px] font-extrabold uppercase", reportTone[report.status])}>{report.status}</span>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-extrabold">{report.summary.branchingAnswers}</p><p className="mt-1 text-[9px] text-white/35">Branches</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-extrabold">{report.summary.productCoverageRate}%</p><p className="mt-1 text-[9px] text-white/35">Product coverage</p></div>
          </div>
        </section>
                  ...
                  return (
                    <article key={node.id} className={cn("w-[220px] shrink-0 rounded-2xl border bg-white p-4 shadow-sm", node.type === "result" ? "border-lime/50" : "border-black/[0.07]")}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", statusTone[node.status])}>{node.status}</span>
                      </div>
                      <h3 className="mt-4 line-clamp-2 text-xs font-extrabold leading-5">{node.label}</h3>
                      <p className="mt-2 line-clamp-3 text-[10px] leading-4 text-black/40">{node.detail}</p>
                      {node.type === "question" && <div className="mt-4 grid grid-cols-3 gap-1 text-center"><span className="rounded-xl bg-canvas p-2"><b className="block text-xs">{node.stats.options}</b><i className="not-italic text-[8px] text-black/35">Options</i></span><span className="rounded-xl bg-canvas p-2"><b className="block text-xs">{node.stats.branchingOptions}</b><i className="not-italic text-[8px] text-black/35">Branches</i></span><span className="rounded-xl bg-canvas p-2"><b className="block text-xs">{node.stats.unmatchedOptions}</b><i className="not-italic text-[8px] text-black/35">Risks</i></span></div>}
                    </article>
                  );
                })}
              </div>
            ...
            <div className="mt-5 grid gap-3 xl:grid-cols-2">
              {report.edges.filter((edge) => edge.answerId).map((edge) => (
                <article key={edge.id} className="rounded-2xl border border-black/[0.07] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold">{edge.label}</h3><p className="mt-1 text-[9px] font-bold text-black/30">{edge.matchType} · weight {edge.weight}</p></div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", statusTone[edge.status])}>{edge.status}</span>
                  </div>
                  <p className="mt-3 text-[10px] leading-4 text-black/45">{edge.detail}</p>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="rounded-full bg-canvas px-2 py-1 text-[8px] font-extrabold text-black/35">{edge.productMatches} matches</span>
                    {edge.skippedQuestions.map((question) => <span key={question} className="rounded-full bg-amber-50 px-2 py-1 text-[8px] font-extrabold text-amber-700">Skips {question}</span>)}
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="mt-5 space-y-3">
              {report.routes.map((route) => (
                <article key={route.id} className="rounded-2xl border border-black/[0.07] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold">{route.label}</h3><p className="mt-1 text-[10px] leading-4 text-black/40">{route.detail}</p></div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", statusTone[route.status])}>{route.status}</span>
                  </div>
                  <div className="mt-3 grid gap-2 xl:grid-cols-3">
                    {route.topProducts.map((product) => <div key={product.id} className="rounded-xl bg-canvas p-3"><p className="truncate text-[10px] font-extrabold">{product.name}</p><p className="mt-1 text-[8px] font-bold text-black/35">{product.price} · score {product.score}</p></div>)}
                    {!route.topProducts.length && <p className="rounded-xl bg-red-50 p-3 text-[10px] font-bold leading-4 text-red-700">No eligible products for this route.</p>}
                  </div>
                  <p className="mt-3 rounded-xl bg-lime/10 p-3 text-[10px] font-bold leading-4 text-black/45">{route.recommendation}</p>
                </article>
              ))}
            </div>
          </section>
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-ink p-5 text-white">
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 text-center">
              <div className="rounded-xl bg-white/[.07] p-4"><p className="text-2xl font-extrabold">{report.summary.routeScenarios}</p><p className="mt-1 text-[9px] text-white/35">Scenarios</p></div>
              <div className="rounded-xl bg-white/[.07] p-4"><p className="text-2xl font-extrabold">{report.summary.passingRoutes}</p><p className="mt-1 text-[9px] text-white/35">Passing</p></div>
            </div>
            <p className="mt-4 text-[10px] font-bold leading-5 text-white/45">Flow Studio uses the same branch resolver and deterministic product matcher as the customer-facing finder runtime.</p>
          </section>
            ...
            <div className="mt-4 space-y-2">
              {questionNodes.map((node, index) => (
                <div key={node.id} className="rounded-2xl bg-canvas p-4">
                  <p className="text-[9px] font-extrabold uppercase tracking-wider text-black/30">Question {index + 1}</p>
                  <h3 className="mt-1 text-xs font-extrabold leading-5">{node.label}</h3>
                  <div className="mt-3 grid grid-cols-4 gap-1 text-center">
                    <span className="rounded-lg bg-white p-2"><b className="block text-xs">{node.stats.options}</b><i className="not-italic text-[8px] text-black/35">Opt</i></span>
                    <span className="rounded-lg bg-white p-2"><b className="block text-xs">{node.stats.matchedOptions}</b><i className="not-italic text-[8px] text-black/35">Match</i></span>
                    <span className="rounded-lg bg-white p-2"><b className="block text-xs">{node.stats.branchingOptions}</b><i className="not-italic text-[8px] text-black/35">Branch</i></span>
                    <span className="rounded-lg bg-white p-2"><b className="block text-xs">{node.stats.unmatchedOptions}</b><i className="not-italic text-[8px] text-black/35">Risk</i></span>
                  </div>
                </div>
              ))}
            </div>
