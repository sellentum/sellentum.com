      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[360px_1fr]">
        <section className="rounded-[28px] border border-black/[0.07] bg-ink p-6 text-white">
          <div className="flex items-center justify-between">
            ...
            <span className={cn("rounded-full px-3 py-1.5 text-[9px] font-extrabold uppercase", snapshot.status === "portable" ? "bg-lime text-ink" : snapshot.status === "review" ? "bg-amber-300/20 text-amber-100" : "bg-red-500/20 text-red-100")}>{snapshot.status}</span>
          </div>
          ...
          {copyError && <p className="mt-3 rounded-xl bg-red-500/10 p-3 text-[10px] font-bold leading-4 text-red-100">{copyError}</p>}
        </section>
        ...
        <main className="space-y-5">
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded-full bg-black/[0.04] px-3 py-1.5 text-[9px] font-extrabold text-black/35">{new Date(snapshot.generatedAt).toLocaleString()}</span>
            </div>
              ...
              {snapshot.exportFiles.map((file) => {
                ...
                return (
                  <button key={file.id} onClick={() => setActiveExport(file.id)} className={cn("rounded-2xl border p-4 text-left transition hover:-translate-y-0.5", activeExport === file.id ? "border-ink bg-ink text-white shadow-sm" : "border-black/[0.07] bg-canvas text-ink")}>
                    ...
                    <p className={cn("mt-2 text-[10px] leading-4", activeExport === file.id ? "text-white/45" : "text-black/40")}>{file.description}</p>
                    <p className={cn("mt-3 text-[8px] font-extrabold uppercase tracking-wider", activeExport === file.id ? "text-lime" : "text-black/30")}>{file.format} · {file.bytes.toLocaleString()} chars</p>
                  </button>
                );
              })}
            </div>
            <div className="mt-5 overflow-hidden rounded-2xl border border-black/[0.07] bg-[#10180f]">
              <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                <div><p className="text-xs font-extrabold text-white">{activeFile?.filename}</p><p className="mt-0.5 text-[9px] font-bold text-white/35">{activeFile?.description}</p></div>
                <div className="flex gap-2">
                  <button onClick={() => copyExport(activeExport)} className="inline-flex items-center gap-1.5 rounded-full bg-white/[0.08] px-3 py-2 text-[10px] font-extrabold text-white"><Clipboard size={12} /> {copied === activeExport ? "Copied" : activeExport === "products" ? "Copy product CSV" : activeExport === "analytics" ? "Copy analytics CSV" : activeExport === "json" ? "Copy JSON archive" : "Copy handoff"}</button>
                  {activeFile && <button onClick={() => downloadExport(activeFile)} className="inline-flex items-center gap-1.5 rounded-full bg-lime px-3 py-2 text-[10px] font-extrabold text-ink"><Download size={12} /> Download</button>}
                </div>
              </div>
              <pre className="max-h-[420px] overflow-auto whitespace-pre-wrap break-words p-5 text-[10px] leading-5 text-lime/80">{activeText}</pre>
            </div>
          </section>
              ...
              {snapshot.checks.map((item) => (
                <article key={item.id} className="rounded-2xl border border-black/[0.07] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="text-xs font-extrabold">{item.label}</h3><p className="mt-1 text-[10px] leading-4 text-black/40">{item.detail}</p></div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", checkTone[item.status])}>{item.status}</span>
                  </div>
                  <Link href={item.href} className="mt-3 inline-flex items-center gap-1 text-[10px] font-extrabold text-moss">{item.action} <ArrowRight size={10} /></Link>
                </article>
              ))}
            </div>
          </section>
            ...
            <div className="mt-4 space-y-2">
              {snapshot.sections.map((section) => (
                <article key={section.id} className="rounded-2xl bg-canvas p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><p className="text-[9px] font-extrabold uppercase tracking-wider text-black/30">{section.label}</p><h3 className="mt-1 text-2xl font-extrabold">{section.count}</h3></div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", checkTone[section.status])}>{section.status}</span>
                  </div>
                  <p className="mt-2 text-[10px] leading-4 text-black/45">{section.detail}</p>
                </article>
              ))}
            </div>
          </section>
