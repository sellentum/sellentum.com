      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[360px_1fr]">
        <section className="rounded-[28px] border border-black/[0.07] bg-ink p-6 text-white">
          ...
          <div className="mt-6 grid grid-cols-2 gap-2 text-center">
            <div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-extrabold">{report.summary.installReady}</p><p className="mt-1 text-xs text-white/35">Ready</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-extrabold">{report.summary.activePartners}</p><p className="mt-1 text-xs text-white/35">Active</p></div>
          </div>
        </section>
        ...
        <main className="space-y-5">
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded-full bg-black/[0.04] px-3 py-1.5 text-xs font-extrabold text-black/35">data-medium=&quot;syndication&quot;</span>
            </div>
                ...
                return (
                  <button key={placement.id} onClick={() => setActiveId(placement.id)} className={cn("rounded-2xl border p-4 text-left transition hover:-translate-y-0.5", activePlacement?.id === placement.id ? "border-ink bg-ink text-white shadow-sm" : "border-black/[0.07] bg-canvas text-ink")}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={cn("rounded-full px-2.5 py-1 text-xs font-extrabold uppercase", statusTone[placement.status])}>{placement.status}</span>
                    </div>
                    <h3 className="mt-4 text-xs font-extrabold">{placement.name}</h3>
                    <p className={cn("mt-2 text-xs leading-4", activePlacement?.id === placement.id ? "text-white/45" : "text-black/40")}>{placement.objective}</p>
                    <p className={cn("mt-3 text-xs font-extrabold uppercase tracking-wider", activePlacement?.id === placement.id ? "text-lime" : "text-black/30")}>{placement.partnerType} · {placement.experience} · {placement.mode}</p>
                  </button>
                );
              })}
            </div>
          ...
          {activePlacement && (
            <section className="rounded-[28px] border border-black/[0.07] bg-white p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <Link href={activePlacement.publicUrl} target="_blank" className="inline-flex items-center gap-1 rounded-full bg-black/[0.04] px-3 py-2 text-xs font-extrabold text-black/45">Open runtime <ExternalLink size={11} /></Link>
              </div>
              <div className="mt-5 grid gap-3 xl:grid-cols-4">
                ...
                ].map(([value, label]) => <div key={String(label)} className="rounded-2xl bg-canvas p-4"><p className="text-2xl font-extrabold">{String(value)}</p><p className="mt-1 text-xs font-extrabold uppercase tracking-wider text-black/30">{String(label)}</p></div>)}
              </div>
              <div className="mt-5 overflow-hidden rounded-2xl border border-black/[0.07] bg-[#10180f]">
                <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                  <div><p className="text-xs font-extrabold text-white">Partner snippet</p><p className="mt-0.5 text-xs font-bold text-white/35">{activePlacement.campaign} · {activePlacement.placement}</p></div>
                  <button onClick={() => copy(activePlacement.snippet, activePlacement.id)} className="inline-flex items-center gap-1.5 rounded-full bg-lime px-3 py-2 text-xs font-extrabold text-ink"><Clipboard size={12} /> {copied === activePlacement.id ? "Snippet copied" : "Copy snippet"}</button>
                </div>
                <pre className="max-h-[300px] overflow-auto whitespace-pre-wrap break-words p-5 text-xs leading-5 text-lime/80">{activePlacement.snippet}</pre>
              </div>
            </section>
          )}
        </main>
            ...
            <>
              <section className="rounded-[28px] border border-black/[0.07] bg-white p-5">
                ...
                <div className="mt-4 rounded-2xl bg-canvas p-4">
                  <span className={cn("rounded-full px-2.5 py-1 text-xs font-extrabold uppercase", activePlacement.nextAction.priority === "critical" ? "bg-red-50 text-red-700" : activePlacement.nextAction.priority === "high" ? "bg-amber-50 text-amber-700" : "bg-lime/35 text-moss")}>{activePlacement.nextAction.priority}</span>
                  <h3 className="mt-3 text-xs font-extrabold leading-5">{activePlacement.nextAction.title}</h3>
                  <p className="mt-2 text-xs leading-4 text-black/45">{activePlacement.nextAction.detail}</p>
                  <Link href={activePlacement.nextAction.href} className="mt-3 inline-flex items-center gap-1 text-xs font-extrabold text-moss">{activePlacement.nextAction.label} <ArrowRight size={10} /></Link>
                </div>
              </section>
              ...
              <section className="rounded-[28px] border border-black/[0.07] bg-white p-5">
                ...
                <div className="mt-4 space-y-2">
                  {activePlacement.acceptanceCriteria.map((item) => <p key={item} className="flex gap-2 text-xs font-bold leading-4 text-black/45"><Check size={13} className="mt-0.5 shrink-0 text-moss" />{item}</p>)}
                </div>
              </section>
              ...
              <section className="rounded-[28px] border border-black/[0.07] bg-ink p-5 text-white">
                ...
                <div className="mt-4 space-y-2">
                  {activePlacement.dataPolicy.map((item) => <p key={item} className="flex gap-2 text-xs font-bold leading-4 text-white/50"><Check size={13} className="mt-0.5 shrink-0 text-lime" />{item}</p>)}
                </div>
              </section>
            </>
          )}
