...
import { BarChart3, BookOpenCheck, Boxes, Check, ChevronRight, CirclePlay, Eye, MousePointerClick, PackagePlus, Rocket, Sparkles, TrendingUp, Upload } from "lucide-react";
  ...
  return (
    <div className="animate-rise">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-black/[0.07] bg-ink p-6 text-white"><div className="flex items-center justify-between"><div><h2 className="text-sm font-extrabold">Experience mix</h2><p className="mt-1 text-xs text-white/35">Finders, advisors and configurators share one catalog.</p></div><Sparkles className="text-lime" size={18} /></div><div className="mt-6 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-white/[.06] p-4"><p className="text-2xl font-extrabold">{quizzes.length}</p><p className="mt-1 text-[9px] text-white/35">Finders</p></div><div className="rounded-xl bg-white/[.06] p-4"><p className="text-2xl font-extrabold">1</p><p className="mt-1 text-[9px] text-white/35">Advisor</p></div><div className="rounded-xl bg-white/[.06] p-4"><p className="text-2xl font-extrabold">{configurators.length}</p><p className="mt-1 text-[9px] text-white/35">Configurators</p></div></div><Link href="/dashboard/settings" className="mt-5 inline-flex items-center gap-2 text-xs font-extrabold text-lime">Choose embed experience <ChevronRight size={13} /></Link></section>
      </div>
    </div>
  );
}
