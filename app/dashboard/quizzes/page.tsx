...
function QuizEditor({ selected, onBack }: { selected: Quiz; onBack: () => void }) {
  ...
  function deleteQuestion(id: string) { if (!confirm("Delete this question and its answers?")) return; const questions = draft.questions.filter((q) => q.id !== id).map((q, i) => ({ ...q, position: i })); setDraft((current) => ({ ...current, questions })); setActiveQuestion(questions[0]?.id || null); }
  function addOption(questionId: string) { const option: AnswerOption = { id: uid("option"), question_id: questionId, label: "New answer", match_type: "tag", match_value: "", weight: 3, position: active?.options.length || 0 }; updateQuestion(questionId, { options: [...(active?.options || []), option] }); }
  ...
  return <div className="-m-4 flex min-h-[calc(100vh-68px)] flex-col sm:-m-7 lg:-m-9">
    ...
    <div className="grid flex-1 lg:grid-cols-[260px_1fr_315px]">
      <aside className="border-b border-black/[0.07] bg-[#eef0eb] p-4 lg:border-b-0 lg:border-r">
        ...
        <div className="mt-5 border-t border-black/[0.07] pt-4"><button onClick={async () => { if (confirm("Delete this finder permanently?")) { await deleteQuiz(draft.id); onBack(); } }} className="flex items-center gap-2 px-2 py-2 text-[10px] font-bold text-red-600"><Trash2 size={12} /> Delete finder</button></div>
      </aside>
      ...
      <main className="min-h-[650px] bg-[#f8f8f5] p-4 sm:p-8 lg:p-10">
        {active ? <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-6 flex items-center justify-between"><div><h3 className="text-sm font-extrabold">Answer options</h3><p className="mt-1 text-[10px] text-black/35">Each option can add a deterministic product-matching signal.</p></div><button onClick={() => addOption(active.id)} className="btn-secondary !px-3 !py-2 text-xs"><Plus size={13} /> Add answer</button></div>
          <div className="mt-3 space-y-3">{active.options.map((option, index) => <div key={option.id} className="rounded-2xl border border-black/[0.07] bg-white p-4 sm:p-5"><div className="flex items-center gap-3"><GripVertical size={14} className="shrink-0 text-black/20" /><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-lime/45 text-[10px] font-extrabold">{String.fromCharCode(65 + index)}</span><input aria-label={`Answer ${index + 1}`} className="min-w-0 flex-1 border-b border-transparent bg-transparent py-1 text-xs font-extrabold outline-none focus:border-black/15" value={option.label} onChange={(e) => updateOption(active.id, option.id, { label: e.target.value })} /><button onClick={() => deleteOption(active.id, option.id)} className="text-black/25 hover:text-red-500"><X size={15} /></button></div><div className="mt-4 grid gap-3 border-t border-black/[0.05] pt-4 sm:grid-cols-[1fr_1fr_80px]"><div><label className="label !text-[9px]">Match using</label><div className="relative"><select className="field appearance-none !py-2.5 text-xs" value={option.match_type} onChange={(e) => updateOption(active.id, option.id, { match_type: e.target.value as MatchType, match_value: "" })}>{Object.entries(typeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-black/25" size={12} /></div></div><div><label className="label !text-[9px]">Match value</label>{option.match_type === "budget_max" ? <input className="field !py-2.5 text-xs" type="number" min="0" value={option.match_value} onChange={(e) => updateOption(active.id, option.id, { match_value: e.target.value })} placeholder="e.g. 100" /> : option.match_type === "none" ? <input className="field !py-2.5 text-xs" disabled value="No product filter" /> : <div className="relative"><input className="field !py-2.5 text-xs" list={`${option.id}-suggestions`} value={option.match_value} onChange={(e) => updateOption(active.id, option.id, { match_value: e.target.value })} placeholder={`Choose ${typeLabels[option.match_type].toLowerCase()}`} /><datalist id={`${option.id}-suggestions`}>{suggestions[option.match_type as "tag" | "category" | "feature"]?.map((item) => <option key={item} value={item} />)}</datalist></div>}</div><div><label className="label !text-[9px]">Weight</label><select className="field !py-2.5 text-xs" value={option.weight} onChange={(e) => updateOption(active.id, option.id, { weight: Number(e.target.value) })}><option value={1}>Low</option><option value={3}>Med</option><option value={5}>High</option></select></div></div></div>)}{!active.options.length && <button onClick={() => addOption(active.id)} className="w-full rounded-2xl border-2 border-dashed border-black/10 p-10 text-center text-xs font-extrabold text-black/35 hover:border-moss/30 hover:text-moss"><Plus className="mx-auto mb-2" size={18} /> Add the first answer</button>}</div>
        </div> : <div className="mx-auto max-w-2xl"><p className="eyebrow text-moss">Welcome screen</p><h2 className="display mt-2 text-4xl">Start with a warm hello</h2><section className="mt-6 rounded-2xl border border-black/[0.07] bg-white p-6"><label className="label">Headline</label><input className="field !text-base font-extrabold" value={draft.welcome_title} onChange={(e) => setDraft({ ...draft, welcome_title: e.target.value })} /><label className="label mt-4">Welcome message</label><textarea className="field min-h-24" value={draft.welcome_message} onChange={(e) => setDraft({ ...draft, welcome_message: e.target.value })} /><label className="label mt-4">Public URL slug</label><div className="flex items-center rounded-xl border border-black/10 bg-canvas px-3"><span className="text-xs text-black/30">/finder/</span><input className="min-w-0 flex-1 bg-transparent py-3 text-xs font-bold outline-none" value={draft.slug} onChange={(e) => setDraft({ ...draft, slug: slugify(e.target.value) })} /></div></section><div className="mt-5 rounded-2xl bg-lime/25 p-4"><p className="flex items-center gap-2 text-xs font-extrabold"><Sparkles size={14} /> A good welcome makes the value clear</p><p className="mt-1.5 text-[10px] leading-4 text-black/45">Tell shoppers how many questions to expect and what they’ll get at the end.</p></div></div>}
      </main>
      ...
      <aside className="hidden border-l border-black/[0.07] bg-white p-5 lg:block">
        ...
        <div className="mt-5 rounded-xl border border-black/[0.07] p-3">
          ...
          <div className="mt-3 space-y-2">
              ...
              return <div key={override.id} className="rounded-xl bg-canvas p-2.5">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[10px] font-extrabold">{overrideLabels[override.action]} · {product?.name || "Missing product"}</p>
                    <p className="mt-0.5 text-[8px] font-bold text-black/35">{override.action === "exclude" ? "Hard filter" : `Adds ${override.action === "pin" ? "top priority" : `+${override.weight} points`}`}{override.note ? ` · ${override.note}` : ""}</p>
                  </div>
            ...
            {!overrides.length && <p className="rounded-xl bg-canvas p-3 text-[9px] font-bold leading-4 text-black/35">No product pins, boosts or exclusions yet.</p>}
          </div>
          {products.length ? <div className="mt-3 space-y-2 border-t border-black/[0.06] pt-3">
            <select value={overrideProductId} onChange={(e) => setOverrideProductId(e.target.value)} className="field !py-2 text-[10px] font-bold">{products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select>
            <div className="grid grid-cols-[1fr_78px] gap-2">
              <select value={overrideAction} onChange={(e) => setOverrideAction(e.target.value as RecommendationOverride["action"])} className="field !py-2 text-[10px] font-bold">{Object.entries(overrideLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
              <select value={overrideWeight} onChange={(e) => setOverrideWeight(Number(e.target.value))} disabled={overrideAction === "exclude"} className="field !py-2 text-[10px] font-bold disabled:opacity-40"><option value={1}>+1</option><option value={3}>+3</option><option value={5}>+5</option><option value={10}>+10</option></select>
            </div>
            <input value={overrideNote} onChange={(e) => setOverrideNote(e.target.value)} className="field !py-2 text-[10px]" placeholder="Internal note, optional" />
            <button onClick={addOverride} className="btn-secondary w-full !px-3 !py-2 text-[10px]"><Plus size={12} /> Add control</button>
          </div> : <p className="mt-3 rounded-xl bg-canvas p-3 text-[9px] font-bold leading-4 text-black/35">Add catalog products before creating merchandising controls.</p>}
        </div>
      </aside>
    </div>
  </div>;
