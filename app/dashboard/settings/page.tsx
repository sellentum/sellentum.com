...
export default function SettingsPage() {
  ...
  const normalizedAllowedDomains = useMemo(() => normalizeAllowedDomains(allowedDomainsText), [allowedDomainsText]);
  ...
  return <div className="animate-rise">
    ...
    <div className="mt-8 grid gap-5 xl:grid-cols-[1fr_.85fr]">
      <div className="space-y-5">
        ...
        <section className="rounded-2xl border border-black/[0.07] bg-white p-5 sm:p-7"><div><h2 className="text-sm font-extrabold">Widget copy</h2><p className="mt-1 text-xs text-black/35">Keep it clear, friendly, and true to your brand voice.</p></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><div><label className="label">Launcher button text</label><input className="field" value={draft.button_text} onChange={(e) => update("button_text", e.target.value)} /></div><div><label className="label">Widget title</label><input className="field" value={draft.widget_title} onChange={(e) => update("widget_title", e.target.value)} /></div><div className="sm:col-span-2"><label className="label">Welcome message</label><textarea className="field min-h-24" value={draft.welcome_message} onChange={(e) => update("welcome_message", e.target.value)} /></div><div><label className="label">Launcher position</label><select className="field" value={draft.launcher_position} onChange={(e) => update("launcher_position", e.target.value)}><option value="bottom-right">Bottom right</option><option value="bottom-left">Bottom left</option></select></div><div><label className="label">Embedded experience</label><select className="field" value={embedType} onChange={(e) => setEmbedType(e.target.value as WidgetEmbedExperience)}><option value="finder">Guided finder</option><option value="assistant">Conversational advisor</option><option value="search">Semantic search</option><option value="configurator">Visual configurator</option></select></div><div className="sm:col-span-2"><label className="label">Embed mode</label><select className="field" value={embedMode} onChange={(e) => setEmbedMode(e.target.value as WidgetEmbedMode)}><option value="modal">Floating launcher modal — lazy-loads when opened</option><option value="inline">Inline iframe — embedded directly in the page</option></select><p className="mt-1 text-xs text-black/30">Modal mode avoids loading the experience iframe until a shopper opens it, keeping view analytics cleaner.</p></div><div className="sm:col-span-2 rounded-2xl border border-moss/10 bg-lime/15 p-4"><div className="flex items-start justify-between gap-4"><div><h3 className="text-xs font-extrabold">Approved storefront domains</h3><p className="mt-1 text-xs text-black/35">Optional during setup, recommended before launch. Once filled, widget analytics are accepted only from matching domains and subdomains.</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-extrabold ${normalizedAllowedDomains.length ? "bg-moss text-white" : "bg-white text-black/45"}`}>{normalizedAllowedDomains.length ? `${normalizedAllowedDomains.length} allowed` : "Open"}</span></div><textarea className="field mt-3 min-h-24 text-xs" value={allowedDomainsText} onChange={(e) => setAllowedDomainsText(e.target.value)} placeholder={"sellentum.com\nwww.yourstore.com"} /><p className="mt-2 text-xs text-black/30">Use one domain per line. Do not include paths; HTTPS and www are cleaned automatically.</p></div><div className="sm:col-span-2 rounded-2xl bg-[#f7f8f4] p-4"><div><h3 className="text-xs font-extrabold">Analytics labels</h3><p className="mt-1 text-xs text-black/35">These labels travel with widget events so Analytics can compare campaigns, pages and placements.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><div><label className="label !text-xs">Source</label><input className="field !py-2.5 text-xs" value={embedSource} onChange={(e) => setEmbedSource(e.target.value)} placeholder="storefront" /></div><div><label className="label !text-xs">Campaign</label><input className="field !py-2.5 text-xs" value={embedCampaign} onChange={(e) => setEmbedCampaign(e.target.value)} placeholder="holiday-guide" /></div><div><label className="label !text-xs">Placement</label><input className="field !py-2.5 text-xs" value={embedPlacement} onChange={(e) => setEmbedPlacement(e.target.value)} placeholder="pdp-bottom" /></div></div></div></div></section>
      </div>
      <div className="space-y-5">
        ...
        {mode === "demo" && <section className="rounded-2xl border border-black/[0.07] bg-white p-5"><div className="flex items-center justify-between gap-4"><div><h2 className="text-xs font-extrabold">Reset demo workspace</h2><p className="mt-1 text-xs text-black/35">Restore the starter products, finder and analytics.</p></div><button onClick={() => { if (confirm("Reset all demo data?")) resetDemo(); }} className="btn-secondary shrink-0 !px-3 !py-2 text-xs"><RotateCcw size={12} /> Reset</button></div></section>}
      </div>
    </div>
  </div>;
}
