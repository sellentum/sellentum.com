        ...
        <aside className="rounded-[28px] border border-black/[0.07] bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-black/35">{answers.length} active path answer{answers.length === 1 ? "" : "s"} · {skippedQuestions.length} skipped</p>
            </div>
          </div>
            ...
            {questionPath.map(({ question, selectedOption }, index) => (
              <section key={question.id} className="rounded-2xl border border-black/[0.07] p-4">
                <div className="flex items-start gap-3">
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-xl bg-lime/45 text-xs font-extrabold text-moss">{index + 1}</span>
                  <div>
                    <h2 className="text-xs font-extrabold leading-5">{question.title}</h2>
                    {question.helper_text && <p className="mt-1 text-xs leading-4 text-black/35">{question.helper_text}</p>}
                  </div>
                </div>
                <div className="mt-3 grid gap-2">
                  {question.options.map((option) => {
                    ...
                    return (
                      <button key={option.id} onClick={() => setSelections((current) => ({ ...current, [question.id]: option.id }))} className={`rounded-xl border px-3 py-2.5 text-left text-xs font-extrabold transition ${selected ? "border-ink bg-ink text-white" : "border-black/10 bg-canvas text-black/50 hover:border-moss/30 hover:bg-white"}`}>
                        <span className="flex items-center justify-between gap-2">
                          {option.label}
                          <span className={`rounded-full px-2 py-0.5 text-xs ${selected ? "bg-lime text-ink" : "bg-white text-black/30"}`}>{option.match_type}</span>
                        </span>
                        {option.match_value && <span className={`mt-1 block text-xs ${selected ? "text-white/45" : "text-black/30"}`}>Value: {option.match_value} · weight {option.weight}</span>}
                        {targetQuestion && <span className={`mt-1 block text-xs ${selected ? "text-lime" : "text-moss"}`}>Branches to question {targetQuestion.position + 1}</span>}
                      </button>
                    );
                  })}
                </div>
          ...
          {skippedQuestions.length > 0 && <div className="mt-5 rounded-2xl border border-dashed border-black/10 bg-canvas p-4">
            <p className="text-xs font-extrabold uppercase tracking-wider text-black/35">Skipped by this branch</p>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {skippedQuestions.map((question) => <span key={question.id} className="rounded-full bg-white px-2.5 py-1 text-xs font-extrabold text-black/35">{question.position + 1}. {question.title}</span>)}
            </div>
          </div>}

          <div className="mt-5 rounded-2xl bg-canvas p-4">
            <p className="text-xs font-extrabold uppercase tracking-wider text-black/35">Buyer profile</p>
            <p className="mt-2 text-xs leading-5 text-black/50">{buyerProfile || "Choose answers to generate a semantic buyer profile."}</p>
            {buyerProfileTerms.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{buyerProfileTerms.map((term) => <span key={term} className="rounded-full bg-white px-2 py-1 text-xs font-extrabold text-black/35">{term}</span>)}</div>}
          </div>
        ...
        <main className="space-y-5">
          <div className="grid gap-4 lg:grid-cols-4">
            {[
              ...
              <div key={label} className="rounded-2xl border border-black/[0.07] bg-white p-4">
                ...
                <p className="mt-1 text-xs font-bold uppercase tracking-wider text-black/30">{label}</p>
              </div>
            ))}
          </div>
            ...
            <div className="grid gap-0 xl:grid-cols-[360px_1fr]">
              <div className="bg-ink p-6 text-white">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className={`rounded-full px-3 py-1.5 text-xs font-extrabold uppercase ${scenarioCoverage.status === "healthy" ? "bg-lime text-moss" : scenarioCoverage.status === "watch" ? "bg-amber-200 text-amber-800" : "bg-red-100 text-red-700"}`}>{scenarioCoverage.status}</span>
                </div>
                ...
                <div className="mt-6 grid grid-cols-[110px_1fr] gap-3">
                  <div className="rounded-2xl bg-white/[.07] p-4 text-center">
                    ...
                    <p className="mt-1 text-xs font-extrabold uppercase tracking-wider text-white/35">Suite score</p>
                  </div>
                    ...
                    {scenarioCoverage.scenarios.slice(0, 4).map((scenario) => (
                      <article key={scenario.id} className="rounded-2xl border border-black/[0.06] bg-[#f8f8f4] p-4">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className={`rounded-full px-2 py-1 text-xs font-extrabold uppercase ${scenarioStatusTone[scenario.status]}`}>{scenario.status}</span>
                        </div>
                        <p className="mt-2 text-xs leading-4 text-black/45">{scenario.detail}</p>
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          {scenario.answers.slice(0, 4).map((answer) => <span key={answer} className="rounded-full bg-white px-2.5 py-1 text-xs font-extrabold text-black/40">{answer}</span>)}
                        </div>
                        <div className="mt-3 grid grid-cols-3 gap-1.5 text-center">
                          <span className="rounded-xl bg-white p-2"><b className="block text-xs">{scenario.eligibleProducts}</b><i className="not-italic text-xs text-black/30">Eligible</i></span>
                          <span className="rounded-xl bg-white p-2"><b className="block text-xs">{scenario.blockedProducts}</b><i className="not-italic text-xs text-black/30">Blocked</i></span>
                          <span className="rounded-xl bg-white p-2"><b className="block text-xs">{scenario.topScore}</b><i className="not-italic text-xs text-black/30">Top score</i></span>
                        </div>
                        <p className="mt-3 text-xs font-bold leading-4 text-moss">{scenario.recommendation}</p>
                      </article>
                    ))}
                  </div>
                      ...
                      {scenarioCoverage.actions.slice(0, 3).map((action) => (
                        <div key={action.id} className="rounded-2xl bg-white/[.07] p-3">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-xs font-extrabold leading-4">{action.title}</p>
                            <span className={`rounded-full px-2 py-0.5 text-xs font-extrabold uppercase ${scenarioPriorityTone[action.priority]}`}>{action.priority}</span>
                          </div>
                          <p className="mt-2 text-xs leading-4 text-white/45">{action.evidence}</p>
                          <p className="mt-2 text-xs leading-4 text-lime">{action.recommendation}</p>
                        </div>
                      ))}
                    </div>
                    <div className="mt-4 rounded-2xl bg-white/[.07] p-3">
                      <p className="text-xs font-extrabold uppercase tracking-wider text-white/35">Product coverage</p>
                      <div className="mt-2 space-y-1.5">
                        {scenarioCoverage.productCoverage.slice(0, 4).map((product) => (
                          <div key={product.productId} className="flex items-center justify-between gap-3 rounded-xl bg-white/[.06] px-3 py-2">
                            <span className="truncate text-xs font-extrabold">{product.productName}</span>
                            <span className="shrink-0 rounded-full bg-lime px-2 py-0.5 text-xs font-extrabold text-moss">{Math.round(product.coverageRate)}%</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="grid gap-0 xl:grid-cols-[1fr_340px]">
              <div className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-lime px-3 py-1.5 text-xs font-extrabold text-moss"><ShieldCheck size={12} className="mr-1 inline" /> No AI selection</span>
                </div>
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {answers.map((answer) => <span key={answer.optionId} className="rounded-full bg-canvas px-3 py-1.5 text-xs font-bold text-black/45">{answer.answer}</span>)}
                </div>
              </div>
              <div className="border-t border-black/[0.07] bg-[#eef1e8] p-6 xl:border-l xl:border-t-0">
                ...
                <p className="mt-2 text-xs leading-5 text-black/45">A product must be active and inside any chosen budget. Matching tags, categories, features and budgets add weighted points. The buyer-profile layer adds semantic/lexical intent points from enriched catalog language. Pins, boosts and exclusions apply as merchant controls. Ties break by lower price, then product name.</p>
                <div className="mt-4 rounded-2xl bg-white/60 p-3">
                  <p className="text-xs font-extrabold uppercase tracking-wider text-black/35">Merchandising controls</p>
                  {overrides.length ? <div className="mt-2 space-y-1.5">{overrides.map((override) => {
                    ...
                    return <p key={override.id} className="text-xs font-bold text-black/45">{override.action.toUpperCase()} · {product?.name || override.product_id}{override.action !== "exclude" ? ` · +${override.weight}` : ""}</p>;
                  })}</div> : <p className="mt-2 text-xs leading-4 text-black/35">No pins, boosts or exclusions are active for this finder.</p>}
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="overflow-hidden rounded-[28px] border border-black/[0.07] bg-white">
            <div className="flex items-center justify-between gap-4 border-b border-black/[0.07] px-5 py-4">
              ...
              <button onClick={copyTrace} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-canvas px-3 py-2 text-xs font-extrabold text-black/45 hover:border-moss/30 hover:text-moss"><Clipboard size={12} /> {copiedTrace ? "Copied" : "Copy trace"}</button>
            </div>
            <div className="grid gap-0 xl:grid-cols-[1fr_360px]">
              <div className="p-5">
                ...
                {trace.topProduct && (
                  <div className="mt-5 rounded-2xl bg-lime/15 p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-extrabold uppercase tracking-wider text-moss">Selected product</p>
                        <h3 className="mt-1 text-xl font-extrabold tracking-[-.05em]">{trace.topProduct.productName}</h3>
                      </div>
                      <span className="rounded-full bg-white px-3 py-1.5 text-xs font-extrabold text-moss">Score {trace.topProduct.score.toFixed(2)}</span>
                    </div>
                    <p className="mt-3 text-xs leading-5 text-black/50">{trace.topProduct.explanation}</p>
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {trace.topProduct.proofPoints.map((point) => <span key={point} className="rounded-full bg-white px-2.5 py-1 text-xs font-extrabold text-black/45">{point}</span>)}
                    </div>
                  </div>
                )}
                ...
                <div className="mt-5 overflow-hidden rounded-2xl border border-black/[0.06]">
                  <div className="grid grid-cols-[1fr_90px_110px] bg-canvas px-4 py-3 text-xs font-extrabold uppercase tracking-wider text-black/35">
                    ...
                    {trace.products.slice(0, 5).map((product) => (
                      <div key={product.productId} className="grid grid-cols-[1fr_90px_110px] gap-3 px-4 py-3">
                        <div>
                          ...
                          <p className="mt-1 line-clamp-1 text-xs text-black/35">{product.blocker || product.decisiveSignals.slice(0, 2).join(" · ") || "No decisive signal"}</p>
                        </div>
                        <p className="text-xs font-extrabold">{product.score.toFixed(2)}</p>
                        <span className={`w-fit rounded-full px-2 py-1 text-xs font-extrabold uppercase ${product.status === "recommended" ? "bg-lime text-moss" : product.status === "blocked" ? "bg-red-50 text-red-600" : "bg-black/5 text-black/35"}`}>{product.status}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <aside className="border-t border-black/[0.07] bg-[#f3f5ef] p-5 xl:border-l xl:border-t-0">
                ...
                <p className="mt-2 text-xs leading-5 text-black/40">Use these to fix weak finder paths before embedding on a storefront.</p>
                <div className="mt-4 space-y-2">
                  {trace.tuningActions.map((action) => (
                    <div key={action.id} className={`rounded-2xl border p-3 ${action.priority === "high" ? "border-red-100 bg-red-50" : action.priority === "medium" ? "border-amber-100 bg-amber-50" : "border-lime/40 bg-lime/15"}`}>
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-xs font-extrabold leading-4">{action.title}</p>
                        <span className={`rounded-full px-2 py-0.5 text-xs font-extrabold uppercase ${action.priority === "high" ? "bg-red-100 text-red-600" : action.priority === "medium" ? "bg-amber-100 text-amber-700" : "bg-lime text-moss"}`}>{action.priority}</span>
                      </div>
                      <p className="mt-1.5 text-xs leading-4 text-black/45">{action.detail}</p>
                    </div>
                  ))}
                </div>
                <pre className="mt-4 max-h-44 overflow-hidden rounded-2xl bg-ink p-3 text-xs leading-4 text-white/55">{tracePayload.slice(0, 1100)}{tracePayload.length > 1100 ? "\n…" : ""}</pre>
              </aside>
            </div>
          </section>
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-white">
            <div className="flex items-center justify-between border-b border-black/[0.07] px-5 py-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-black/35">Same scorer used by the customer-facing finder.</p>
              </div>
                  <span className="rounded-full bg-black/5 px-3 py-1 text-xs font-bold text-black/35">Top 3 · branch-aware path</span>
            </div>
                ...
                return (
                  <article key={audit.product.id} className="grid gap-5 p-5 xl:grid-cols-[260px_1fr]">
                    <div>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {recommendedRank >= 0 && <span className="rounded-full bg-lime px-2 py-0.5 text-xs font-extrabold text-moss">#{recommendedRank + 1}</span>}
                          </div>
                          <p className="mt-1 text-xs text-black/35">{audit.product.category} · {formatCurrency(audit.product.price)}</p>
                        </div>
                      </div>
                      <div className="mt-4">
                        <div className="flex items-center justify-between text-xs font-extrabold text-black/35"><span>Score</span><span>{audit.score.toFixed(2)}</span></div>
                      ...
                      {!audit.eligible && <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-bold text-red-600">{audit.blockedReason}</p>}
                    </div>
                    ...
                    <div className="grid gap-2 lg:grid-cols-2">
                      {audit.signals.map((signal) => (
                        <div key={`${audit.product.id}-${signal.answer}-${signal.matchType}`} className={`rounded-2xl border p-3 ${signal.matched ? "border-lime/50 bg-lime/15" : "border-black/[0.06] bg-canvas/60"}`}>
                          <div className="flex items-start justify-between gap-3">
                            <p className="text-xs font-extrabold leading-4">{signal.answer}</p>
                            <span className={`flex items-center gap-1 rounded-full px-2 py-1 text-xs font-extrabold ${signal.matched ? "bg-lime text-moss" : "bg-white text-black/30"}`}>{signal.matched ? <CheckCircle2 size={9} /> : <XCircle size={9} />}{signal.contribution.toFixed(1)}</span>
                          </div>
                          <p className="mt-1.5 text-xs leading-4 text-black/40">{signal.note}</p>
                        </div>
                      ))}
                    </div>
                  </article>
