      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-[380px_1fr]">
        <section className="rounded-[30px] border border-black/[0.07] bg-ink p-7 text-white">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            ...
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.enrichedProducts}</p><p className="mt-1 text-[8px] text-white/35">Enriched</p></div>
            <div className="rounded-2xl bg-white/[0.06] p-3"><p className="text-xl font-extrabold">{report.summary.downstreamExperiences}</p><p className="mt-1 text-[8px] text-white/35">Live uses</p></div>
          </div>
        </section>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-[9px] font-extrabold uppercase text-black/30">{stage.score}% · {stage.status}</p>
                        </div>
                      </div>
                      <span className={cn("grid h-8 w-8 place-items-center rounded-xl", stageTone[stage.status])}><StatusIcon size={15} /></span>
                    </div>
                    <p className="mt-3 text-[10px] leading-4 text-black/45">{stage.detail}</p>
                    <p className="mt-3 rounded-xl bg-white px-3 py-2 text-[9px] font-bold leading-4 text-black/45">{stage.evidence}</p>
                  </Link>
                );
              })}
            </div>
                ...
                <article key={source.id} className="rounded-2xl border border-black/[0.07] bg-canvas p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-[9px] font-extrabold uppercase text-black/30">{source.count} item{source.count === 1 ? "" : "s"}</p>
                    </div>
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", stageTone[source.status])}>{source.status}</span>
                  </div>
                  <p className="mt-3 text-[10px] leading-4 text-black/45">{source.detail}</p>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {source.contract.map((item) => <span key={`${source.id}-${item}`} className="rounded-full bg-white px-2 py-1 text-[8px] font-extrabold text-black/35">{item}</span>)}
                  </div>
                </article>
              ))}
            </div>
              ...
              {report.fieldCoverage.map((field) => (
                <article key={field.id} className="rounded-2xl bg-canvas p-4">
                  <div className="flex items-center justify-between gap-3">
                    <span className={cn("rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase", stageTone[field.status])}>{field.status}</span>
                    <span className="text-[9px] font-extrabold text-black/30">{field.filled}/{field.total}</span>
                  </div>
                  <p className="display mt-5 text-3xl">{field.coverage}%</p>
                  <h3 className="mt-1 text-xs font-extrabold leading-4">{field.label}</h3>
                  <p className="mt-2 text-[10px] leading-4 text-black/45">{field.detail}</p>
        ...
        <aside className="space-y-5">
            ...
            <div className="mt-4 space-y-2">
              {report.consumers.map((consumer) => (
                <Link key={consumer.label} href={consumer.href} className="flex items-start gap-3 rounded-2xl bg-canvas p-4 transition hover:bg-white">
                  ...
                  <span>
                    <span className="flex items-center gap-2 text-xs font-extrabold">{consumer.label}<span className="rounded-full bg-white px-2 py-0.5 text-[8px] text-black/35">{consumer.count}</span></span>
                    <span className="mt-1 block text-[10px] leading-4 text-black/40">{consumer.detail}</span>
                  </span>
                </Link>
              ))}
            </div>
          ...
          <section className="rounded-[28px] border border-black/[0.07] bg-white p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-center">
              <div className="rounded-2xl bg-canvas p-4"><ShieldCheck className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.discoveryReadyProducts}</p><p className="mt-1 text-[8px] font-bold text-black/30">Ready SKUs</p></div>
              <div className="rounded-2xl bg-canvas p-4"><BarChart3 className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.productsWithDemand}</p><p className="mt-1 text-[8px] font-bold text-black/30">Demand SKUs</p></div>
              <div className="rounded-2xl bg-canvas p-4"><Layers3 className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.categories}</p><p className="mt-1 text-[8px] font-bold text-black/30">Categories</p></div>
              <div className="rounded-2xl bg-canvas p-4"><AlertTriangle className="mx-auto text-moss" size={16} /><p className="mt-3 text-2xl font-extrabold">{report.summary.duplicateGroups}</p><p className="mt-1 text-[8px] font-bold text-black/30">Duplicates</p></div>
            </div>
          </section>
        </aside>
      </div>
