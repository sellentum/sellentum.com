...
export function buildExplanationGroundingReport({
  ...
  if (!audits.length) {
    addAction(actions, {
      ...
      detail: "Sellentum needs at least one launchable finder path and eligible product before it can audit result-card explanations.",
  ...
  if (!openaiConfigured) {
    addAction(actions, {
      ...
      detail: "OpenAI is not configured, so Sellentum will use safe deterministic explanation copy.",
