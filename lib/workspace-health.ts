...
export const expectedWorkspaceSchema: Array<Omit<WorkspaceSchemaContract, "status" | "missingColumns">> = [
  ...
  { table: "rate_limit_buckets", purpose: "Shared server-side API throttling buckets for Vercel/serverless public runtimes.", requiredColumns: ["key_hash", "request_count", "reset_at", "updated_at"] },
];
...
const expectedWorkspaceFunctions = [
  ...
  { name: "check_rate_limit", purpose: "Shared Supabase RPC used by public API guardrails for production rate limiting." },
];
...
export function buildWorkspaceHealthReport({ mode, source = "client-store", products, quizzes, configurators, events, settings, schemaSql }: {
  ...
  const checks = [
    ...
    check("supabase-rpcs", "schema", "Supabase RPC coverage", !normalizedSchemaSql ? "warn" : missingFunctions.length ? "fail" : "pass", !normalizedSchemaSql ? "Schema SQL was not available to prove required production RPCs." : missingFunctions.length ? `Missing ${missingFunctions.map((item) => item.name).join(", ")}.` : "Transactional builder saves and shared production rate limiting are backed by Supabase RPC functions.", expectedWorkspaceFunctions.map((item) => `${item.name}: ${item.purpose}`).join(" "), "/dashboard/data-contract"),
