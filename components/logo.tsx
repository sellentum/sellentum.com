...
export function Logo({ href = "/", light = false }: { href?: string; light?: boolean }) {
  return (
    <Link href={href} className="inline-flex items-center gap-2.5 font-extrabold tracking-[-0.03em]" aria-label="Sellentum home">
      ...
      <span className={light ? "text-white" : "text-ink"}>sellentum</span>
    </Link>
  );
}
