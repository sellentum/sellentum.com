  ...
  return (
    <div className={cn("rounded-xl border border-black/[0.06] bg-white p-3", compact && "p-2.5")} data-feedback-product={productId}>
      <div className="flex items-center justify-between gap-3">
        <p className={cn("text-[9px] font-extrabold uppercase tracking-wider text-black/30", compact && "text-[8px]")}>Was this match useful?</p>
        {selected && <span className="inline-flex items-center gap-1 rounded-full bg-lime/35 px-2 py-1 text-[8px] font-extrabold text-moss"><Check size={10} /> Saved</span>}
      </div>
        ...
        {options.map(({ sentiment, reason, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={cn(
                "inline-flex items-center justify-center gap-1.5 rounded-full border px-3 py-2 text-[9px] font-extrabold transition disabled:opacity-60",
