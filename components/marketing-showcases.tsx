      ...
      <div className="relative overflow-hidden rounded-[32px] border border-black/10 bg-white shadow-[0_35px_100px_rgba(28,39,32,.16)]">
        <div className="flex items-center justify-between border-b border-black/[0.07] px-6 py-4">
          ...
          <div className="flex rounded-full bg-[#f0f2ec] p-1">
            <button onClick={() => setView("shopper")} className={`rounded-full px-4 py-2 text-[10px] font-extrabold transition ${view === "shopper" ? "bg-white shadow-sm" : "text-black/40"}`}>Shopper experience</button>
            <button onClick={() => setView("merchant")} className={`rounded-full px-4 py-2 text-[10px] font-extrabold transition ${view === "merchant" ? "bg-white shadow-sm" : "text-black/40"}`}>Merchant workspace</button>
          </div>
          <span className="hidden rounded-full bg-lime/35 px-3 py-1.5 text-[9px] font-extrabold text-moss sm:block">Live product demo</span>
        </div>
        ...
        {view === "shopper" ? (
          <div className="grid min-h-[525px] lg:grid-cols-[.86fr_1.14fr]">
            ...
            <div className="bg-[linear-gradient(135deg,#f7f8f3_0%,#eff3e2_55%,#fff2e8_100%)] p-7 sm:p-10">
              <div className="flex items-center justify-between"><p className="eyebrow text-moss">Your current best match</p><span className="rounded-full bg-white px-3 py-1.5 text-[9px] font-extrabold shadow-sm">94% match</span></div>
              <div className="mt-5 grid overflow-hidden rounded-[24px] border border-black/10 bg-white shadow-xl sm:grid-cols-[1.05fr_.95fr]">
                <div className="relative min-h-[315px] overflow-hidden bg-[#eceee9]"><img src={match.image} alt={match.name} className="absolute inset-0 h-full w-full object-cover transition duration-500" /><span className="absolute left-4 top-4 rounded-full bg-lime px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-wider">Best match</span></div>
                <div className="flex flex-col p-6"><p className="text-[9px] font-extrabold uppercase tracking-[.14em] text-moss">Running shoes</p><h4 className="mt-2 text-2xl font-extrabold leading-tight tracking-[-.04em]">{match.name}</h4><p className="mt-1 text-lg font-extrabold">{formatCurrency(match.price)}</p><div className="mt-5 rounded-xl bg-[#f3f5ef] p-4"><p className="flex items-center gap-1.5 text-[9px] font-extrabold text-moss"><Sparkles size={11} /> Why it fits</p><p className="mt-2 text-[11px] leading-5 text-black/55">{match.reason}</p></div><div className="mt-4 flex flex-wrap gap-1.5">{match.tags.map((tag) => <span key={tag} className="rounded-full border border-black/10 px-2.5 py-1 text-[9px] font-bold text-black/45">{tag}</span>)}</div><button className="mt-auto rounded-full bg-ink px-5 py-3 text-xs font-extrabold text-white">See my full results →</button></div>
              </div>
            </div>
          </div>
        ) : (
          <div className="grid min-h-[525px] bg-[#f4f5f1] lg:grid-cols-[220px_1fr]">
            <aside className="hidden border-r border-black/[0.07] bg-[#eaede6] p-4 lg:block"><p className="px-3 py-3 text-[9px] font-extrabold uppercase tracking-wider text-black/30">Workspace</p>{[["Overview", BarChart3], ["Products", Boxes], ["Product finders", Footprints], ["Configurators", PackagePlus], ["Analytics", Eye], ["Brand & embed", Code2]].map(([label, Icon], index) => { const MenuIcon = Icon as typeof BarChart3; return <div key={String(label)} className={`mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-xs font-bold ${index === 2 ? "bg-ink text-white" : "text-black/45"}`}><MenuIcon size={15} className={index === 2 ? "text-lime" : ""} />{String(label)}</div>; })}</aside>
            <div className="p-6 sm:p-9"><div className="flex items-center justify-between"><div><p className="eyebrow text-moss">Product finder</p><h3 className="mt-2 text-2xl font-extrabold tracking-[-.04em]">Find your perfect pair</h3></div><button className="rounded-full bg-ink px-4 py-2.5 text-[10px] font-extrabold text-white">Publish finder</button></div><div className="mt-7 grid gap-5 xl:grid-cols-[1fr_280px]"><div className="rounded-2xl border border-black/[0.07] bg-white p-5"><p className="text-xs font-extrabold">Conversation flow</p><div className="mt-5 space-y-3">{["Where will you wear them most?", "What matters most underfoot?", "What’s your comfortable budget?"].map((question, index) => <div key={question} className="flex items-center gap-4 rounded-xl border border-black/[0.07] p-4"><span className="grid h-8 w-8 place-items-center rounded-xl bg-lime/45 text-xs font-extrabold">{index + 1}</span><span className="flex-1 text-xs font-extrabold">{question}</span><span className="rounded-full bg-[#f1f3ed] px-2 py-1 text-[8px] font-bold text-black/35">{index === 2 ? "Budget" : index === 1 ? "Feature" : "Tag"}</span><Settings2 size={14} className="text-black/25" /></div>)}</div><button className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-black/15 py-3 text-[10px] font-extrabold text-moss">+ Add question</button></div><div className="rounded-2xl bg-ink p-5 text-white"><p className="text-[9px] font-extrabold uppercase tracking-wider text-lime">Live performance</p><p className="mt-7 text-[10px] text-white/40">Completion rate</p><p className="mt-1 text-4xl font-extrabold tracking-[-.06em]">52%</p><div className="mt-6 h-1.5 rounded-full bg-white/10"><div className="h-full w-[52%] rounded-full bg-lime" /></div><div className="mt-8 grid grid-cols-2 gap-2"><div className="rounded-xl bg-white/[.06] p-3"><Eye size={13} className="text-lime" /><p className="mt-3 text-lg font-extrabold">1.2k</p><p className="text-[8px] text-white/35">Views</p></div><div className="rounded-xl bg-white/[.06] p-3"><MousePointerClick size={13} className="text-lime" /><p className="mt-3 text-lg font-extrabold">184</p><p className="text-[8px] text-white/35">Buy clicks</p></div></div></div></div></div>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid overflow-hidden rounded-[30px] border border-black/10 bg-white shadow-soft lg:grid-cols-[.92fr_1.08fr]">
        <div className="p-8 sm:p-12 lg:p-14"><p className="eyebrow text-moss">{mode === "shoppers" ? "Guided discovery" : "Repeatable expertise"}</p><h3 className="mt-4 max-w-xl text-4xl font-extrabold leading-[1.03] tracking-[-.055em] sm:text-5xl">{mode === "shoppers" ? <>Turn a few answers into <span className="text-moss">a confident choice.</span></> : <>Turn product knowledge into <span className="text-moss">a scalable sales experience.</span></>}</h3><p className="mt-5 max-w-lg text-sm leading-6 text-black/50">{mode === "shoppers" ? "Replace endless browsing with a short, thoughtful conversation that narrows a complex catalog to the products that genuinely fit." : "Give merchandising teams a visual builder, reliable matching controls, live brand settings and the signals they need to keep improving."}</p><div className="mt-7 grid gap-2 sm:grid-cols-2">{(mode === "shoppers" ? ["Questions that feel useful", "1–3 focused recommendations", "Clear reasons for every match", "Direct paths to purchase"] : ["CSV and manual catalog tools", "Tag, category, feature and budget rules", "One-click publishing", "Views, completions and buying intent"]).map((item) => <div key={item} className="flex items-center gap-2 rounded-xl bg-[#f3f5ef] px-3 py-3 text-[10px] font-extrabold"><span className="grid h-5 w-5 place-items-center rounded-full bg-lime"><Check size={11} /></span>{item}</div>)}</div><a href={mode === "shoppers" ? "/finder/quiz_footwear" : "/login"} className="mt-8 inline-flex items-center gap-2 rounded-full bg-ink px-6 py-3.5 text-xs font-extrabold text-white">{mode === "shoppers" ? "Try the live finder" : "Explore the workspace"}<ArrowRight size={14} /></a></div>
        <div className="relative min-h-[540px] overflow-hidden bg-[radial-gradient(circle_at_75%_20%,rgba(217,255,97,.8),transparent_35%),linear-gradient(135deg,#edf0e8,#fff2e7)] p-8 sm:p-12">{mode === "shoppers" ? <div className="grid h-full place-items-center"><div className="w-full max-w-md rotate-2 rounded-[26px] border border-black/10 bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-[10px] font-extrabold"><span className="grid h-7 w-7 place-items-center rounded-lg bg-ink text-lime"><Sparkles size={12} /></span>Your matches</span><span className="rounded-full bg-lime px-2 py-1 text-[8px] font-extrabold">3 found</span></div><div className="mt-5 grid grid-cols-2 gap-3">{demoMatches.slice(0, 2).map((item, i) => <div key={item.name} className="overflow-hidden rounded-xl border border-black/10"><div className="relative h-32"><img src={item.image} alt="" className="h-full w-full object-cover" />{i === 0 && <span className="absolute left-2 top-2 rounded-full bg-lime px-2 py-1 text-[7px] font-extrabold">BEST MATCH</span>}</div><div className="p-3"><p className="text-[10px] font-extrabold">{item.name}</p><p className="mt-1 text-[9px] text-black/40">{formatCurrency(item.price)}</p></div></div>)}</div><div className="mt-4 rounded-xl bg-[#f3f5ef] p-3"><p className="text-[8px] font-extrabold text-moss">✦ WHY IT FITS</p><p className="mt-1 text-[9px] leading-4 text-black/45">Matched to the use, feel and budget preferences you shared.</p></div></div></div> : <div className="grid h-full place-items-center"><div className="w-full max-w-md -rotate-2 rounded-[26px] border border-black/10 bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><div><p className="text-[8px] font-extrabold uppercase tracking-wider text-moss">Analytics</p><p className="mt-1 text-sm font-extrabold">Product finder performance</p></div><BarChart3 size={18} className="text-moss" /></div><div className="mt-5 grid grid-cols-3 gap-2">{[["1,284", "Views"], ["667", "Completed"], ["184", "Buy clicks"]].map(([value, label]) => <div key={label} className="rounded-xl bg-[#f3f5ef] p-3"><p className="text-lg font-extrabold">{value}</p><p className="text-[8px] text-black/35">{label}</p></div>)}</div><div className="mt-5 flex h-44 items-end gap-2">{[38, 55, 45, 68, 62, 79, 58, 85, 73, 92].map((height, i) => <div key={i} className="flex-1 rounded-t bg-moss" style={{height:`${height}%`}}><div className="h-[35%] rounded-t bg-lime" /></div>)}</div></div></div>}</div>
      </div>
    </div>
  );
}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {capabilities.map(({ title, copy, icon: ItemIcon }, index) => (
          <button key={title} onClick={() => setActive(index)} className={`min-h-[150px] rounded-2xl border p-5 text-left transition hover:-translate-y-0.5 ${active === index ? "border-ink bg-white shadow-lg ring-1 ring-ink" : "border-black/10 bg-white/55 hover:bg-white"}`}>
            ...
            <span className="mt-1.5 block text-[10px] leading-4 text-black/40">{copy.split(".")[0]}.</span>
          </button>
        ))}
      </div>
      <div className="overflow-hidden rounded-[28px] border border-black/10 bg-white shadow-soft">
        <div className="relative min-h-[340px] overflow-hidden bg-[radial-gradient(circle_at_70%_15%,rgba(217,255,97,.9),transparent_34%),linear-gradient(135deg,#e9ede5,#fff0e5)] p-7">
          ...
          <div className="relative mx-auto max-w-lg rounded-2xl border border-black/10 bg-white/90 p-5 shadow-xl backdrop-blur">
            <div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center rounded-xl bg-ink text-lime"><Icon size={18} /></span><span className="rounded-full bg-lime px-2.5 py-1 text-[8px] font-extrabold uppercase tracking-wider">Findly engine</span></div>
            {item.visual === "catalog" && <div className="mt-5 space-y-2">{["Terra Trail Runner", "Aero City Knit", "Pulse Tempo Pro"].map((name, i) => <div key={name} className="flex items-center gap-3 rounded-xl border border-black/[0.07] p-3"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#eef0eb]"><Boxes size={13} /></span><span className="flex-1 text-[10px] font-extrabold">{name}</span><span className="text-[8px] text-black/30">{i === 0 ? "trail · cushioned" : i === 1 ? "city · light" : "road · speed"}</span></div>)}</div>}
            {item.visual === "builder" && <div className="mt-5 space-y-2">{["Where will you wear them?", "What matters underfoot?", "What is your budget?"].map((name, i) => <div key={name} className="flex items-center gap-3 rounded-xl border border-black/[0.07] p-3"><span className="grid h-7 w-7 place-items-center rounded-lg bg-lime/50 text-[9px] font-extrabold">{i + 1}</span><span className="text-[10px] font-extrabold">{name}</span></div>)}</div>}
            {item.visual === "rules" && <div className="mt-5 rounded-xl border border-black/[0.07] p-4"><p className="text-[9px] font-extrabold">If answer is “Weekend trails”</p><div className="mt-3 flex items-center gap-2 text-[9px]"><span className="rounded-lg bg-[#eef0eb] px-2.5 py-2">Product tag</span><span>is</span><span className="rounded-lg bg-lime/50 px-2.5 py-2 font-extrabold">trail</span><span className="ml-auto rounded-full bg-ink px-2 py-1 text-[7px] text-white">High weight</span></div></div>}
            {item.visual === "ai" && <div className="mt-5 space-y-3"><div className="ml-auto max-w-[80%] rounded-2xl rounded-br-sm bg-ink p-3 text-[9px] leading-4 text-white">I need something cushioned for wet weekend trails.</div><div className="max-w-[90%] rounded-2xl rounded-bl-sm bg-[#eef0eb] p-3 text-[9px] leading-4 text-black/55"><b className="text-moss">Why Terra fits:</b> Trail grip and soft cushioning match the conditions and feel you described.</div></div>}
            {item.visual === "configurator" && <div className="mt-5 space-y-2"><div className="rounded-xl bg-ink p-4 text-white"><p className="text-[8px] font-extrabold uppercase tracking-wider text-lime">Build your trail kit</p><p className="mt-2 text-2xl font-extrabold tracking-[-.05em]">£166</p><p className="mt-1 text-[9px] text-white/40">Terra + wet trails + care kit</p></div>{["Wet trails & mud", "Maximum cushioning", "Waterproof care kit"].map((option, i) => <div key={option} className="flex items-center gap-3 rounded-xl border border-black/[0.07] p-3"><span className="grid h-6 w-6 place-items-center rounded-lg bg-lime/50 text-[9px] font-extrabold"><Check size={10} /></span><span className="flex-1 text-[10px] font-extrabold">{option}</span><span className="text-[8px] text-black/30">{i === 0 ? "compatible" : "selected"}</span></div>)}</div>}
            {item.visual === "embed" && <div className="mt-5 rounded-xl bg-ink p-4 font-mono text-[8px] leading-5 text-lime">&lt;script<br />&nbsp;&nbsp;src=&quot;findly.app/widget.js&quot;<br />&nbsp;&nbsp;data-experience=&quot;configurator&quot;<br />&nbsp;&nbsp;data-id=&quot;trail-kit&quot;<br />&nbsp;&nbsp;async<br />&gt;&lt;/script&gt;</div>}
        ...
        <div className="p-7 sm:p-9">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-3">{item.points.map((point) => <span key={point} className="flex items-center gap-2 text-[10px] font-extrabold"><Check size={12} className="text-moss" />{point}</span>)}</div>
        </div>
      </div>
    </div>
  );
