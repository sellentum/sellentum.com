  ...
  return (
    <main className="grid min-h-screen bg-white lg:grid-cols-[1fr_.9fr]">
      <section className="flex min-h-screen flex-col px-5 py-6 sm:px-10 lg:px-16">
        ...
        <div className="mx-auto my-auto w-full max-w-[430px] py-16"><p className="eyebrow text-moss">Welcome to Findly</p><h1 className="display mt-3 text-5xl leading-none">{title}</h1><p className="mt-4 text-sm leading-6 text-black/50">{copy}</p>{children}</div>
        <p className="text-center text-[11px] text-black/30">By continuing, you agree to our Terms and Privacy Policy.</p>
      </section>
      <aside className="noise relative m-3 hidden overflow-hidden rounded-[32px] bg-ink p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <p className="relative text-xs text-white/30">Findly · Guided selling, made lighter</p>
      </aside>
    </main>
  );
}
